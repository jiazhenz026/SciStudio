...
- **FR-010** requires every Image-domain ProcessBlock in
  ``scieasy-blocks-imaging`` to comply, with the audit recorded at
...
def test_mode_a_add_scalar_propagates_ome() -> None:
    ...
    from scieasy_blocks_imaging.math.scalar_ops import AddScalar
...
def test_mode_b_resize_factor_half_doubles_physical_pixel_size() -> None:
    ...
    from scieasy_blocks_imaging.preprocess.geometry import Resize
...
def test_mode_b_axis_split_preserves_in_plane_sampling() -> None:
    ...
    from scieasy_blocks_imaging.preprocess.axis_ops import AxisSplit
...
def test_mode_c_compute_registration_transform_has_no_ome_carrier() -> None:
    ...
    from scieasy_blocks_imaging.types import Transform
