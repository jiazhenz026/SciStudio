...
from scistudio.panels.fallbacks import core_panel_specs, dataframe_panel
...
def _registry(*specs: PanelSpec, with_core: bool = True) -> PanelRegistry:
    reg = PanelRegistry()
    if with_core:
        for s in core_panel_specs():
            reg.register(s)
    for s in specs:
        reg.register(s)
    return reg
...
def test_package_exact_wins_over_core_parent() -> None:
    """US2: Image -> package panel; plain Array -> core (FR-003 tier 4 vs 8)."""
...
def test_parent_fallback_when_no_exact() -> None:
    """A child type with no exact panel falls back to its parent's (FR-003 tier 6)."""
    pkg_parent = _spec("pkg.array", OwnerKind.PACKAGE, "Array")
    router = PreviewRouter(_registry(pkg_parent))
    # FancyImage has no exact panel; Array (parent) does, at package tier.
...
def test_user_item_panel_does_not_capture_collection() -> None:
    """A user-tier single-item panel must not capture a collection target."""
...
def test_collection_routes_to_collection_capable_panel() -> None:
    """US4 scenario 1: a collection-capable package panel wins for Collection[Image]."""
...
def test_collection_falls_back_to_core_collection() -> None:
    """US4 scenario 2: no collection-specific panel -> core collection fallback."""
    router = PreviewRouter(_registry())
    spec = router.resolve(_collection_target("Image", ("DataObject", "Array", "Image")))
    assert spec.previewer_id == "core.collection.basic"


def test_collection_with_item_panel_still_falls_back_to_core_collection() -> None:
    """Regression (ADR-048 FR-003 / US4 scenario 2): a single-item panel must
    NOT capture a collection target. Collection[Image] with an Image *item*
    panel present (e.g. the imaging package's single-image viewer) but no
    collection-capable panel must resolve to the core collection fallback,
...
def test_collection_with_item_panel_and_no_core_raises() -> None:
    """A collection that has only a single-item panel and no core collection
