"""Contract tests for the panel-owned DataFrame table cache.

ADR-048 / issue #1598 moved the table-paging LRU cache down out of
``scistudio.api.runtime._preview_cache`` into
``scistudio.panels._table_cache`` so the panel subsystem no longer
...
from scistudio.panels import _table_cache
