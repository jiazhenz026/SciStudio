...
from scistudio.panels.fallbacks import dataframe_panel
from scistudio.panels.models import OwnerKind, PanelSpec
from scistudio.panels.registry import PanelRegistry


def _spec(previewer_id: str, owner: OwnerKind = OwnerKind.PACKAGE, target: str = "Image") -> PanelSpec:
    return PanelSpec(
...
def test_factory_returning_non_list_is_diagnosed() -> None:
    ...
    assert any("expected list[PanelSpec]" in d for d in reg.diagnostics)
...
def test_factory_returning_non_spec_item_is_skipped() -> None:
    ...
    assert any("non-PanelSpec" in d for d in reg.diagnostics)


def test_raising_factory_is_diagnosed_not_fatal() -> None:
    reg = PanelRegistry()

    def boom() -> list[PanelSpec]:
...
def test_companion_package_entry_point_discovers_get_previewers(
    ...
    """Block/type packages can provide panels even when panel metadata is stale."""
    fake_module = types.ModuleType("scistudio_blocks_fake")
    fake_module.get_previewers = lambda: [_spec("pkg.fake.viewer")]  # type: ignore[attr-defined]
    monkeypatch.setitem(sys.modules, "scistudio_blocks_fake", fake_module)
...
def test_load_packages_activates_installed_plugin_roots_for_entry_points(
    ...
    # panel scan alone, so the seam to patch moved there with it.
    monkeypatch.setattr(
