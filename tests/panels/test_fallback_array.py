"""Provider-level tests for the generic array panel (ADR-048 FR-013/FR-014).

These exercise :func:`array_panel` directly with a stub
...
from scistudio.panels.data_access import ArrayPlane, PreviewDataAccess, SliceAxis
from scistudio.panels.fallbacks import array_panel
...
def test_array_panel_emits_numeric_heatmap_payload() -> None:
    request = _request({"_storage": {"backend": "zarr", "path": "x"}})
    env = array_panel(request)
...
def test_array_panel_forwards_typed_request_storage() -> None:
...
def test_array_panel_reads_axis_indices_from_query() -> None:
...
def test_array_panel_ignores_malformed_axis_indices() -> None:
