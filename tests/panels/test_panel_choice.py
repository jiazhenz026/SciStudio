"""The person's chosen panel per type — storage and routing (#2049).
...
from scistudio.panels.models import OwnerKind, PanelSpec, PreviewTarget, TargetKind
...
def registry() -> PanelRegistry:
    """A registry with one panel per tier, all claiming ``Probe``."""
    reg = PanelRegistry()
...
def test_the_ladder_is_untouched_when_nothing_is_chosen(registry: PanelRegistry) -> None:
...
def test_a_choice_wins_over_the_whole_ladder(registry: PanelRegistry, chosen: str) -> None:
    """Including over the project tier, which the ladder would otherwise pick."""
    registry.set_panel_choices({"Probe": chosen})
    assert PreviewRouter(registry).resolve(ITEM).previewer_id == chosen


def test_choosing_what_the_ladder_would_have_picked_changes_nothing(registry: PanelRegistry) -> None:
    registry.set_panel_choices({"Probe": "probe.project"})
    assert PreviewRouter(registry).resolve(ITEM).previewer_id == "probe.project"


def test_a_choice_on_one_type_does_not_govern_another(registry: PanelRegistry) -> None:
    registry.set_panel_choices({"Image": "probe.package"})
    assert PreviewRouter(registry).resolve(ITEM).previewer_id == "probe.project"
...
def test_a_choice_does_not_reach_types_that_merely_descend_from_the_chosen_one(
    ...
    registry.set_panel_choices({"Series": "series.core"})
    assert PreviewRouter(registry).resolve(ITEM).previewer_id == "probe.project"
...
def test_a_choice_naming_an_unregistered_panel_falls_back(registry: PanelRegistry) -> None:
    """The realistic case: the package that provided it was uninstalled."""
    registry.set_panel_choices({"Probe": "gone.after.uninstall"})
    assert PreviewRouter(registry).resolve(ITEM).previewer_id == "probe.project"


def test_a_choice_for_an_unrelated_type_falls_back(registry: PanelRegistry) -> None:
    """Bounded to the target's type chain, so a choice reorders the ladder's
    candidates but can never widen them to a panel that claims something
    else entirely."""
    registry.register(_spec("image.viewer", OwnerKind.PACKAGE, target_type="Image"))
    registry.set_panel_choices({"Probe": "image.viewer"})
    assert PreviewRouter(registry).resolve(ITEM).previewer_id == "probe.project"


def test_choosing_an_ancestors_panel_is_honoured(registry: PanelRegistry) -> None:
    """Picking core's plain ``Series`` view for a ``Probe`` is a real preference,
    not a mistake, so the chain bound admits it."""
    registry.register(_spec("series.core", OwnerKind.CORE, target_type="Series"))
    registry.set_panel_choices({"Probe": "series.core"})
...
def test_a_single_item_choice_is_not_used_for_a_collection() -> None:
    ...
    panel, so falling back is visible rather than coincidentally identical.
    """
    reg = PanelRegistry()
    ...
    reg.set_panel_choices({"Probe": "probe.single"})
...
def test_a_collection_capable_choice_is_honoured_for_a_collection() -> None:
    ...
    reg.set_panel_choices({"Probe": "probe.batch.chosen"})
...
def test_the_project_default_still_only_breaks_a_same_tier_tie() -> None:
    ...
    Two package panels tie on priority; the project default picks one. That
...
def test_a_choice_and_a_project_default_do_not_meet(registry: PanelRegistry) -> None:
    """When a choice applies, the ladder does not run, so FR-005 never arbitrates."""
    registry.set_project_default("Probe", "probe.project")
    registry.set_panel_choices({"Probe": "probe.package"})
