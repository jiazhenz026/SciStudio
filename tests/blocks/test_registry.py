...
class TestAdr050PackageSchemaContract:
    """ADR-050 #1698: the registry preserves package config-schema UI metadata.

    The square node + BottomPanel refactor removes inline node config but MUST
    keep ``ui_priority`` / ``ui_widget`` on package config fields so BottomPanel
    can order fields and pick widgets (FR-029/SC-010). These assert the contract
    at the ``_spec_from_class`` / scanned-registry boundary, complementing the
    API-level tests in ``tests/api/test_blocks.py``.

    Spec: docs/specs/adr-050-canvas-node-readability.md FR-029, SC-010.
    """

    def test_scanned_package_block_keeps_ui_priority_in_config_schema(self) -> None:
        """A package process block keeps integer ``ui_priority`` on every config field."""
        registry = BlockRegistry()
        registry.scan()
        spec = next(
            (s for s in registry.all_specs().values() if s.type_name == "lcms.pool_size_normalize"),
            None,
        )
        assert spec is not None, "lcms.pool_size_normalize not registered"
        props = spec.config_schema.get("properties", {})
        assert props, "lcms.pool_size_normalize lost its config properties"
        assert all("ui_priority" in prop for prop in props.values()), (
            "registry dropped ui_priority from a package config field"
        )

    def test_scanned_package_app_block_keeps_ui_widget_in_config_schema(self) -> None:
        """A package app block keeps ``ui_widget`` widget hints for BottomPanel."""
        registry = BlockRegistry()
        registry.scan()
        spec = next(
            (s for s in registry.all_specs().values() if s.type_name == "imaging.napari"),
            None,
        )
        assert spec is not None, "imaging.napari not registered"
        props = spec.config_schema.get("properties", {})
        widgets = {name: prop.get("ui_widget") for name, prop in props.items()}
