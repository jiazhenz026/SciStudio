...
``scieasy.cli.install._scieasy_command_for_env``). That invocation needs
``scieasy/__main__.py`` to exist, otherwise Python errors out with
``No module named scieasy.__main__`` and the MCP client reports
"scieasy MCP server failed" with no useful diagnostic.
...
def test_python_dash_m_scieasy_dispatches_to_cli() -> None:
    ...
    # Spot-check that the expected subcommands appear so a future regression
    # that ships a __main__.py pointing at an unrelated entry-point would
    # fail loudly.
