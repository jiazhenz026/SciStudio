...
3. **MCP tool registry contract**: The MCP server must always expose
   exactly 36 tools (ADR-040 §3.1 + Addendum 5 + ADR-048 SPEC 2 plot + edit_workflow #1912
...
# + list_miniapps (#2441, renamed list_panels in #2445) + wait_for_answers (#2447)
# + 14 external-audience tools (ADR-055 Spec 2,
# #2279; served by the WebMCP bridge, hidden from the local socket transport): 55 tools total.
...
_MCP_EXPECTED_COUNT = 55


def test_mcp_server_exposes_55_tools() -> None:
    """ADR-040 §3.1 + Addendum 5 + ADR-048 SPEC 2 + #1912 + #1947 + ADR-053 FR-011 + ADR-054 + #2441 + #2447 + ADR-055 Spec 2: MCP server must expose 55 tools.
