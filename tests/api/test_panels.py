"""Routed panel session API tests (ADR-048 FR-007 / FR-028 / FR-029)."""
...
def _prefer_fixture_package(monkeypatch: pytest.MonkeyPatch) -> None:
    """Ensure the in-repo fixture panel package is the discovered plugin.

    Issue #1770: the real imaging package was decoupled out of core. These
    API tests exercise *core* panel routing / asset serving against the
...
def test_create_session_unknown_target_returns_error_envelope(client: TestClient, opened_project: Path) -> None:
    ...
    package nor a collection-capable panel — the core collection fallback
...
def test_image_session_serializes_first_class_frontend_manifest(
    ...
    monkeypatch.setattr(importlib.metadata, "entry_points", _entry_points)
    # Rebuild the preview service so the fixture panels are registered.
    runtime.refresh_preview_service()
...
def test_collection_session_lists_items(
    ...
    # Use a base type (DataFrame) that no package panel claims, so this test
    # deterministically exercises the core collection fallback whether or not any
    # package panel is registered (an Image panel would otherwise route
...
def test_collection_image_child_resource_uses_catalog_storage(
    ...
    """Collection item previews must route through the catalog-backed Image panel."""
    import numpy as np
    import tifffile
...
def test_imaging_panel_asset_served_from_companion_package_entry_point(
    client: TestClient,
    runtime: ApiRuntime,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Imaging viewer assets remain available when panel entry-point metadata is stale."""
    _prefer_fixture_package(monkeypatch)
