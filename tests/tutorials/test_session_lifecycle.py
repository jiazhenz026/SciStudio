...
class _Provisioner:
    """Creates a directory that :func:`tutorial_project_exists` recognizes.
...
def _runtime(
    ...
    return TutorialRuntime(
        ...
        # harness that let them disagree would be modeling a state the product
...
def test_a_condition_satisfied_while_the_learning_center_is_closed_is_not_lost(
    ...
    and recorded by something the user is not looking at. Reopening — modeled
...
def test_evaluation_continues_when_the_frontend_disconnects(
    ...
    Modeled as the thing a disconnect actually is: events keep arriving on the
