...
* ``tools/list`` enumerates all 27 tools (25 baseline + finish_ai_block + ADR-040 Addendum 5 get_active_workflow_context).
...
async def _test_mcp_server_initialize_tools_list_and_call(tmp_path: Path) -> None:
    ...
    try:
        ...
        # tools/list — expect all 27 (25 baseline + finish_ai_block from ADR-035
