...
from scistudio.agent_provisioning import (
...
def test_install_project_agent_assets_fresh_project(tmp_project_dir: Path) -> None:
    ...
    assert result.version == SCISTUDIO_PROVISION_VERSION
    ...
    # 16 skill files (8 names, 2 trees) — flat layout per ADR-040 Addendum 1;
    # 8 = 1 base + 7 task skills (incl. ADR-048 scistudio-write-plot and
    # ADR-054 §8.3 scistudio-write-panel).
    skill_files = [
        p
        for p in result.written
        if (p.startswith(".claude/skills/") or p.startswith(".agents/skills/")) and p.endswith("/SKILL.md")
    ]
    assert len(skill_files) == 16, f"expected 16 skill files, got {len(skill_files)}: {skill_files}"
...
def test_install_project_agent_assets_partial_failure(tmp_project_dir: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("scistudio.agent_provisioning._orchestrate.write_codex_config", _boom)
...
def test_version_marker_written(tmp_project_dir: Path) -> None:
    """``.claude/.scistudio-provision-version`` contains SCISTUDIO_PROVISION_VERSION."""
    install_project_agent_assets(tmp_project_dir, force=False)
    marker = tmp_project_dir / ".claude" / ".scistudio-provision-version"
