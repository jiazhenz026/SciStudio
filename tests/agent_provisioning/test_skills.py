...
def test_write_skills_creates_parent_dirs(tmp_path: Path) -> None:
    ...
    assert (target / ".claude" / "skills" / "scistudio").is_dir()
    assert (target / ".agents" / "skills" / "scistudio").is_dir()
...
def test_write_skills_placeholder_for_missing_source(tmp_project_dir: Path) -> None:
    ...
    On this implementation branch, the Skills track (_skills/scistudio/<name>/)
    ...
    The base ``scistudio`` skill falls back to the legacy monolithic source.
    """
    write_skills(tmp_project_dir, force=False)
    # 5 task-scoped placeholders contain the TODO marker.
    for task_name in ("scistudio-build-workflow", "scistudio-debug-run"):
