...
export const api = {
  ...
  // ADR-033 / T-ECA-303: embedded coding agent surface.
  getAIStatus: () => apiFetch<AIStatusResponse>("/api/ai/status"),
  // ADR-033 / T-ECA-110: REST fallback for permission decisions when
  // the WS channel is unavailable.
  sendPermissionDecision: (body: {
    chat_id: string;
    request_id: string;
    decision: "approve" | "deny";
    reason?: string;
  }) =>
    apiFetch<void>("/api/ai/permission-decision", {
      method: "POST",
      headers: JSON_HEADERS,
      body: JSON.stringify(body),
    }),
  // #718(b): workflow PUT with optional If-Match header for conflict
  // detection. TODO: the backend If-Match handling lands in PR #734
  // (workflow versioning); once merged, wire the 412 handler in the
  // workflow auto-save call site (see frontend/src/store/workflowSlice).
  updateWorkflowVersioned: (
    workflowId: string,
    ...
    return apiFetch<WorkflowResponse>(`/api/workflows/${encodeURIComponent(workflowId)}`, {
      method: "PUT",
      headers,
      body: JSON.stringify(body),
    });
...
/** Mirrors `scieasy.api.schemas.ProviderStatusResponse`. */
export interface AIStatusResponse {
  providers: AIProviderStatus[];
}

/** Mirrors `scieasy.api.schemas.ProviderStatusItem`. */
