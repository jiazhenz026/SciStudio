...
function boot(standalone = false, sample = {}) {
  ...
  const win: Record<string, unknown> = {
    location: { href: "https://example.test/panel/index.html" },
  };
  win.parent = standalone ? win : {};
  win.addEventListener = (type: string, cb: (event: unknown) => void) => {
    listeners[type] = cb;
  };
  ...
  const init = (kind = "preview", sender = win.parent) =>
    listeners.message?.({
      source: sender,
      ports: [port],
      data: {
  ...
  const response = (payload: unknown = null) => {
    const call = port.postMessage.mock.lastCall![0];
    port.onmessage?.({ data: { v: 1, type: "result", id: call.id, payload } });
  };
...
describe("dependency-free SDK", () => {
  ...
  it("applies themes and rejects pending requests on disposal", async () => {
    const { api, port, init, response, root } = boot();
    init();
    const ready = api.ready();
    await Promise.resolve();
    response();
    ...
    port.onmessage?.({
      data: { v: 1, type: "theme", payload: { mode: "light", tokens: { "--ss-ink": "0 0 0" } } },
    });
  ...
  it("exposes only writeBack in interactive mode and transfers save buffers", async () => {
    ...
    response();
    await ready;
    expect(api.read).toBeUndefined();
    expect(api.open).toBeUndefined();
    const data = new ArrayBuffer(4);
    const saving = api.save({ name: "x", mime: "application/octet-stream", data });
