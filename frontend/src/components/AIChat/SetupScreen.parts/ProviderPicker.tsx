...
/** Whether this option can be chosen — the one predicate the ordering and `disabled` read. */
function isSelectable(s: ProviderStatus): boolean {
  return s.available;
}
...
function orderedProviders(providers: readonly ProviderStatus[]): ProviderStatus[] {
  return [
    ...providers.filter((p) => isSelectable(p)),
    ...providers.filter((p) => !isSelectable(p)),
  ];
}
