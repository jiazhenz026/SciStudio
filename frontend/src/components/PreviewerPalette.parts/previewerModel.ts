// Pure Previewers tab model — tier grouping, ordering, filtering, and the
 ...
 * previewer vanish; it collects here instead, in the same A→Z remainder every
 * real package name lands in.
 */
export const PACKAGES_SECTION_ID = "Packages";

/**
 * The section a previewer belongs to — its discovery tier, then its package.
 * Same shape as `typeSectionIdFor` on the type side, with no `custom` origin:
 * the previewer registry has no fourth drop-in kind (an unresolvable drop-in
 * never reaches the listing at all).
 */
export function previewerSectionIdFor(previewer: PreviewerSpecSummary): string {
  switch (previewer.owner_kind) {
    case "project":
    ...
    default:
      return previewer.owner_name || PACKAGES_SECTION_ID;
  }
}

/**
 * FR-037-style teaching copy, the previewer-side counterpart of the type
 * hints: for a user who has never heard of a previewer drop-in, the empty
...
/**
 * The ordered head of the previewer section list. This Project and My Library
 ...
 * previewers close the list as the A→Z remainder — the same shape the Blocks
 * and Data types tabs use (`buildSections` always emits the remainder last),
 * so all three left-panel catalogues read identically.
 */
export const PREVIEWER_SECTION_SLOTS: readonly SectionSlot[] = [
...
export function ownerKindLabel(kind: PreviewerSpecSummary["owner_kind"]): string {
  switch (kind) {
    case "project":
      return "This Project";
    case "user":
...
/** Text a previewer is searched by: id, target type, owner, capabilities. */
function previewerHaystack(previewer: PreviewerSpecSummary): string {
  return `${previewer.previewer_id} ${previewer.target_type} ${previewer.owner_name} ${previewer.capabilities.join(" ")}`;
}

/** Text-search filter. Previewers have no facet chips (the tier sections
 *  already are the grouping a person scans by). */
export function filterPreviewers(
  previewers: readonly PreviewerSpecSummary[],
  search: string,
): PreviewerSpecSummary[] {
  return filterItems(previewers, search, previewerHaystack);
}

/** True when a search term is narrowing the list. */
export function isFilteringPreviewers(search: string): boolean {
  return search.trim().length > 0;
}

const byId = (a: PreviewerSpecSummary, b: PreviewerSpecSummary): number =>
  a.previewer_id.localeCompare(b.previewer_id);

/**
 * Build the ordered Previewers sections from the visible previewers. Under an
 ...
 * the current query.
 */
export function buildPreviewerSections(
  previewers: readonly PreviewerSpecSummary[],
  search: string,
): PreviewerSection[] {
  const visible = filterPreviewers(previewers, search);
  const slots = isFilteringPreviewers(search)
    ? withoutEmptyHints(PREVIEWER_SECTION_SLOTS)
    : PREVIEWER_SECTION_SLOTS;
  return buildSections(visible, previewerSectionIdFor, slots, byId);
}
...
export function choiceForType(
  choices: readonly PreviewerChoice[],
  targetType: string,
): PreviewerChoice | null {
  return choices.find((choice) => choice.target_type === targetType) ?? null;
}

/**
 * Choices whose previewer is not registered right now (`available: false`).
 * No card exists for a stale choice's previewer, so without surfacing them
 * here a stale preference would be invisible — and un-clearable — from the
 * tab whose job is showing these choices.
 */
export function staleChoices(choices: readonly PreviewerChoice[]): PreviewerChoice[] {
