/**
 * ADR-048 SPEC 1 — same-origin dynamic panel module loading (FR-022).
 *
 * Validates a {@link PanelFrontendManifest}, dynamically `import()`s its ESM
 * module from a *same-origin* URL only, reads the named export, checks it is a
 * {@link PanelModule} with a compatible API version, and mounts it into a
...
import type { PanelFrontendManifest } from "../../types/api";
import {
  type PreviewHostApi,
  type PanelInstance,
  type PanelModule,
 ...
 * panel assets under `/api/previews/assets/...`, so a legitimate
...
function injectManifestCss(manifest: PanelFrontendManifest): void {
  for (const href of manifest.css ?? []) {
    if (!isSameOriginModuleUrl(href)) continue;
    if (document.querySelector(`link[data-panel="${manifest.previewer_id}"][href="${href}"]`)) {
      continue;
    }
    const link = document.createElement("link");
    link.rel = "stylesheet";
    link.href = href;
    link.dataset.panel = manifest.previewer_id;
...
export async function mountDynamicPanel(
  manifest: PanelFrontendManifest,
  ...
  if (!isApiVersionCompatible((exported as PanelModule).apiVersion)) {
    return {
      ok: false,
      reason: "api_version_mismatch",
      message: `module apiVersion ${(exported as PanelModule).apiVersion} is incompatible with host`,
    };
  }
  ...
  try {
    const instance = (exported as PanelModule).mount(container, hostApi);
