...
_DOTTED_SCIEASY_RE = re.compile(r"\bscieasy(?:\.[A-Za-z_][A-Za-z0-9_]*)+\b")
_SRC_PATH_RE = re.compile(r"\bsrc/scieasy/[A-Za-z0-9_./-]+(?:\.py)?\b")
...
def _is_repo_path_reference(token: str) -> bool:
    return token.startswith("src/scieasy/")


def _is_dotted_repo_reference(token: str) -> bool:
    return bool(_DOTTED_SCIEASY_RE.fullmatch(token)) and len(token.split(".")) >= 3
...
def _iter_prose_references(prose: str) -> Iterable[tuple[str, int]]:
    ...
    for regex in (_SRC_PATH_RE, _DOTTED_SCIEASY_RE):
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            for alias in node.names:
                name = alias.name
                if not name.startswith("scieasy."):
                    continue
            ...
            if not module.startswith("scieasy."):
                continue
