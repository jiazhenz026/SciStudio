...
``@mcp.tool(name=...)`` and declare Pydantic result models with
``next_step: str`` on write-class tools.
...
# ---------------------------------------------------------------------------
# Run-level error capture (preserved from ADR-033 era).
# ---------------------------------------------------------------------------
...
def _ensure_error_subscriber() -> None:
    ...
    (e.g. a standalone-mode runtime stub used by tests), this is a
...
def _collect_run_errors(run_id: str) -> list[dict[str, Any]]:
    """Return the captured block errors for ``run_id``."""
    return [
...
# ---------------------------------------------------------------------------
# Helpers (preserved from ADR-033 era).
# ---------------------------------------------------------------------------
...
def _looks_like_inline_yaml(s: str) -> bool:
    """Heuristic: starts with ``name:``/``workflow:``/... ⇒ inline YAML."""
    stripped = s.lstrip()
...
def _get_workflow_runtime() -> Any:
    ...
    Production: the FastAPI ``ApiRuntime`` exposes ``start_workflow``.
    Tests: a stub provides the same attribute.
    """
    ctx = get_context()
...
class ListTypesResult(BaseModel):
    ...
    types: list[TypeEntry] = Field(description="Registered DataObject subtypes.")
...
class WorkflowDefinitionEnvelope(BaseModel):
    ...
    # Allow arbitrary fields from dataclasses.asdict so we don't lose
    # structure that's specific to a particular WorkflowDefinition layout.
    model_config = {"extra": "allow"}
...
async def list_blocks() -> list[BlockSpecEnvelope]:
    ...
    return [BlockSpecEnvelope(**_spec_to_dict(spec)) for spec in specs.values()]
...
async def list_types() -> ListTypesResult:
    ...
    Use when:
      - You're choosing a port ``type`` for a new block.
...
async def write_workflow(
    ...
    yaml: str = Field(description="Full workflow YAML content; schema-validated before write."),
) -> WriteWorkflowResult:
...
async def run_workflow(
    ...
    # Clear any stale errors from a prior failed run with the same id.
...
async def cancel_run(
    ...
    run = runs[run_id]
    event_bus = getattr(getattr(run, "scheduler", None), "_event_bus", None)
...
async def get_run_status(
    ...
    error_dicts: list[dict[str, Any]] = _collect_run_errors(run_id)
    if state == "failed" and not error_dicts and task is not None:
        try:
            exc = task.exception()
        except Exception:
            exc = None
    ...
    errors = [BlockErrorEntry(**e) for e in error_dicts]
...
def _resolve_ai_block_run_dir() -> Path | None:
    ...
    Resolution order:
      ...
      1. ``MCPContext.ai_block_run_dir`` when present and non-None.
      2. ``SCIEASY_AI_BLOCK_RUN_DIR`` env var when set and pointing at
...
async def finish_ai_block(
    outputs: Annotated[
        ...
        Field(
            description=(
                "{port_name: absolute_or_project_relative_path} for every declared "
                "output port. None is accepted and treated as {} (FileWatcher path "
                "will validate via expected_path)."
            ),
        ),
    ] = None,
