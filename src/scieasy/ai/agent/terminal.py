...
inside SciEasy via xterm.js — the WebSocket route in
:mod:`scieasy.api.routes.ai_pty` calls ``PtyProcess.read`` / ``write``
        ...
        if sys.platform == "win32":
            try:
                ...
                raise RuntimeError(
                    ...
                    "Reinstall scieasy: `pip install -e .` (pywinpty is a "
    ...
    *,
    ...
    Argv:
        ``["claude", "--append-system-prompt", "@<prompt_file>",
            "--mcp-config", "<project>/.scieasy/mcp.json"]``
        plus ``--dangerously-skip-permissions`` when ``dangerous``.

    The system prompt file is composed via
    :func:`scieasy.ai.agent.system_prompt.compose_system_prompt` and
