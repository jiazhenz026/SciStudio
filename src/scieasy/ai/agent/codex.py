...
class CodexSession:
    ...
    async def send_user_message(self, content: str) -> None:
        ...
        # Codex P2 review: reset the cancelled flag at the start of every
        # new turn so a cancel on turn N does not silently swallow a
        # non-zero exit on turn N+1 in ``stream_events``.
        self._cancelled = False
        ...
        # ``CREATE_NEW_PROCESS_GROUP`` only exists in the ``subprocess``
        # module on Windows; on POSIX we use ``start_new_session=True``
        # (set inside ``_spawn``). ``getattr`` keeps mypy happy on
        # non-Windows without paying a runtime cost on Windows.
