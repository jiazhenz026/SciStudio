...
Writes ``<project>/.codex/config.toml`` containing:
  ...
  * A single ``[mcp_servers.scieasy]`` block (and nested
    ``[mcp_servers.scieasy.env]`` table) pinning ``SCIEASY_PROJECT_DIR``
    to the absolute project path. Rendered by
    :func:`scieasy.cli.install._render_codex_block` so this auto-installed
    TOML is byte-identical to what ``scieasy install --target codex
    --scope project`` emits.
...
# arrive under their fully-qualified ``mcp__scieasy__*`` name, identical
...
_PRE_HOOKS: tuple[tuple[str, str, str], ...] = (
    ("^Bash$", "deny_scieasy_cli.py", "Checking SciEasy CLI usage"),
    (
    ...
    (
        "^(Edit|Write|MultiEdit|apply_patch|Bash|mcp__scieasy__scaffold_block)$",
...
_POST_HOOKS: tuple[tuple[str, str, str], ...] = (
    ...
    (
        "^(Edit|Write|MultiEdit|apply_patch|mcp__scieasy__scaffold_block)$",
