"""SciEasy source-version-control subsystem (ADR-039).

This package wraps a **bundled portable git CLI binary** (MinGit on Windows,
static-built git on macOS/Linux) so every SciEasy project is a real git
...
watcher. The REST layer at ``scieasy.api.routes.git`` calls into this
...
from scieasy.core.versioning.git_binary import BundledGitMissing, GitBinary
