...
Before authoring a new block, you MUST call `mcp__scieasy__list_blocks`
...
The base class lives in `scieasy.blocks.base.block`. Subclasses MUST
...
`Collection`, subclass `scieasy.blocks.process.process_block.ProcessBlock`
...
1. Call `mcp__scieasy__list_types` to enumerate registered data types.
...
3. If no registered type fits, declare a new `DataObject` subclass in
   your plugin's `scieasy.types` entry point. Do NOT silently widen
   to `DataObject`.

**Two enforcement layers complement this rule**:

- `mcp__scieasy__scaffold_block` returns a `warnings: list[str]`
...
- Call `mcp__scieasy__list_blocks` FIRST. Reuse if any block's
  contract matches.
- Call `mcp__scieasy__list_types` before selecting port types. Pick
