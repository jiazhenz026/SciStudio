...
# Development references: FR-021, FR-023, FR-026, FR-044, docs/specs/adr-053-work-import.md.
...
"""The template's answer substitution points, in display order.
...
class BriefTemplateError(RuntimeError):
    ...
    so a half-composed brief is unrecoverable.
    """
...
def load_brief_template() -> str:
    """Return the bundled ``brief_template.md`` text verbatim."""
    # Maintainer context:
...
def compose_brief(context: ImportSessionContext) -> str:
    ...
    Returns the template text with the dialog's answers substituted into its
    "What they told us" section. Every other character is the
    ...
    Raises:
        BriefTemplateError: if the template's answers section does not present
            exactly the expected substitution points, or one of them is not
    ...
    #         in either form composition understands.
    # Development references: FR-023, FR-026.
...
def _answer_or_fallback(answer: str | None, marker: str) -> str:
    """The user's answer, or the template's wording for not having one."""
...
def _fallback(marker: str) -> str:
    ...
    keeps the skip wording identical to the bundled template.
    """
    # Maintainer context:
    # Taking the wording from the template rather than restating it here is what
    # keeps the contract's skip wording identical to §4.6's.
    # Development references: FR-021.
