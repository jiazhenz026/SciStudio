...
def _expected_skill_paths() -> list[str]:
    """Return the 22 skill-file paths the skills sub-step is expected to write.

    1 base + 10 task skills (including MiniApp, panel, and data-type authoring and
    the ``scistudio-use-gui`` operation guide) across 2 provider trees = 22.
    """
    # Development references: ADR-048, ADR-054, FR-028.
    names = [
        "scistudio",
