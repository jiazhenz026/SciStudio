...
Collection paging is caller controlled: pass `hasMore`, `loading`, and `onLoadMore` to show the same bounded Show more control used by the core shell. Series data can include `source_indices` alongside `nonfinite_positions` so decimated curves preserve gaps in original source coordinates.
