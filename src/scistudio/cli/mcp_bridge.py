...
def _attached_socket_path(project_dir: Path) -> Path:
    ...
    return project_dir / ".scistudio" / "mcp.sock"
...
async def _run_standalone(project_dir: Path | None) -> int:
    ...
    from scistudio.ai.agent.mcp.runtime import start_inprocess_server, stop_inprocess_server
...
def run(socket: str | None) -> int:
    ...
    1. Resolve the active project dir from ``SCISTUDIO_PROJECT_DIR``.
    ...
    3. Otherwise enter **standalone mode**: spawn an in-process
       :class:`MCPServer` via :mod:`scistudio.ai.agent.mcp.runtime` and
       proxy through that.
        ...
        try:
            if sys.platform == "win32":
                ...
                if port_file.exists():
                    ...
                    print(
                        f"scistudio mcp-bridge: --socket {socket} unreachable (no .port file)",
                        file=sys.stderr,
                    )
                    return 2
            else:
            ...
            print(f"scistudio mcp-bridge: cannot connect to {socket}: {exc}", file=sys.stderr)
            return 2
        try:
    ...
    if project_dir is None:
        print(
            "scistudio mcp-bridge: SCISTUDIO_PROJECT_DIR is not set. The bridge needs "
            "a project root to scope its block/type registries. Re-run "
            "`scistudio install` or set SCISTUDIO_PROJECT_DIR in your MCP client config.",
            file=sys.stderr,
        )
        return 2
...
def _typer_command(
    ...
    with the bridge's return code so ``scistudio mcp-bridge`` exits with
...
def register(app: typer.Typer) -> None:
    ...
    Called from :mod:`scistudio.cli.main` at module import time so the
    existing ``scistudio`` console script gains the subcommand without a
    new ``[project.scripts]`` entry. Mirrors the pattern used by
    :func:`scistudio.cli.hook_bridge.register`.
    """
    app.command(
        "mcp-bridge",
        help="Proxy MCP JSON-RPC frames between the calling MCP client and SciStudio.",
    )(_typer_command)
