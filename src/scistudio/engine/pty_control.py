...
def request_pty_tab(spec: PtyTabSpec) -> str:
    ...
    2. Otherwise, look up ``SCISTUDIO_ENGINE_API_URL`` (engine's own
    ...
    * Missing env var → :class:`RuntimeError` with diagnostic.
    ...
    api_url = os.environ.get("SCISTUDIO_ENGINE_API_URL")
    if not api_url:
        raise RuntimeError(
            "request_pty_tab: SCISTUDIO_ENGINE_API_URL is not set. "
...
def notify_block_pty_event(
    ...
    3. Otherwise POST to
       ``{SCISTUDIO_ENGINE_API_URL}/api/ai/pty/internal/notify`` with the
    ...
    if not api_url:
        ...
        logger.warning(
            "notify_block_pty_event: SCISTUDIO_ENGINE_API_URL is not set; skipping notify for run %s",
            block_run_id,
        )
        return
