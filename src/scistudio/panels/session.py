...
def _activated_package_import_roots(owner_kind: OwnerKind) -> Iterator[None]:
    """Activate installed package import roots around a package panel call.

    #2112: the same deferred-import gap
    :func:`scistudio.blocks.io._unified_dispatch._activated_package_import_roots`
    closes for delegated IO. A package panel module is importable at render
...
class PreviewSessionManager:
    ...
    def __init__(
        ...
        *,
        ...
        from scistudio.core.dropins import panel_import_roots, panel_scan_dirs

        self._dropin_import_roots = panel_import_roots(project_dir)
        scan_dirs = panel_scan_dirs(project_dir)
        ...
        self._project_panels_root = scan_dirs[0] if project_dir is not None else None
        self._user_panels_root = scan_dirs[-1]
    ...
    def read_session(self, session_id: str) -> PreviewEnvelope:
        ...
        Raises :class:`UnknownPanelError` (code ``unknown_previewer``) when
    ...
    def read_resource(self, session_id: str, resource_id: str, params: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        * ``slot:<name>`` / ``item:<idx>`` — child routing for composite slots
        ...
        Returns a JSON-safe dict. Raises :class:`UnknownPanelError` for an
    ...
    def _render(
        ...
        if provider is None:
            return self._error_envelope(
                ...
                f"panel {spec.previewer_id!r} has no backend provider",
                previewer_id=spec.previewer_id,
    ...
    def _resolve_provider(self, spec: PanelSpec) -> PreviewProvider | None:
        return self._provider_from_decl_scoped(spec, spec.backend_provider)

    def _resolve_resource_provider(self, spec: PanelSpec) -> PreviewResourceProvider | None:
        return self._provider_from_decl_scoped(spec, spec.resource_provider)

    def _owning_panel_root(self, owner_kind: OwnerKind) -> Path | None:
        """Return the drop-in directory *owner_kind* specs are scanned from."""
        if owner_kind is OwnerKind.PROJECT:
            return self._project_panels_root
        if owner_kind is OwnerKind.USER:
            return self._user_panels_root
        return None

    def _provider_from_decl_scoped(self, spec: PanelSpec, provider: ProviderT | str | None) -> ProviderT | None:
        """Resolve a provider declaration, scoped to the tier that owns *spec*.

        A drop-in panel (project or user tier) may declare its provider as
        ...
        owning_root = self._owning_panel_root(spec.owner_kind)
        ...
        # other tier's panel directory is deliberately absent, so sibling
    ...
    def _require_spec(self, previewer_id: str) -> PanelSpec:
        spec = self._registry.get(previewer_id)
        if spec is None:
            raise UnknownPanelError(
                f"Unknown panel: {previewer_id}",
    ...
    def _export_plot_resource(self, session: PreviewSession, params: dict[str, Any]) -> dict[str, Any]:
        ...
        if session.target.kind is not TargetKind.PLOT_ARTIFACT and session.previewer_id != "core.plot.basic":
            raise ProviderError(
                f"resource 'export' is not available for panel {session.previewer_id!r}",
...
def _dropin_provider_from_decl(dotted: str, owning_root: Path) -> Callable[..., Any] | None:
    ...
    try:
        ...
        synth_name = f"_scistudio_panel_provider_{path.stem}_{path.stat().st_mtime_ns}"
...
def _missing_spec(previewer_id: str) -> PanelSpec:
