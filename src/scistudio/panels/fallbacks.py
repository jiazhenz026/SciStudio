"""Core fallback panel providers (ADR-048 FR-012 .. FR-019).

Each provider is a :data:`scistudio.panels.models.PreviewProvider` callable
...
Providers (and their core :class:`PanelSpec` ids):

* ``core.dataframe.basic`` — :func:`dataframe_panel`
* ``core.array.basic`` — :func:`array_panel` (GENERIC numeric only;
  NO image-domain LUT/OME/channel/label semantics, FR-013/FR-014)
* ``core.series.basic`` — :func:`series_panel` (chart + table, FR-015)
* ``core.text.basic`` — :func:`text_panel` (FR-016)
* ``core.artifact.basic`` — :func:`artifact_panel`
* ``core.composite.basic`` — :func:`composite_panel` (slot inventory, FR-017)
* ``core.collection.basic`` — :func:`collection_panel` (FR-009, tier-7 fallback)
* ``core.plot.basic`` — :func:`plot_panel` (PNG/JPEG/SVG/PDF, FR-018/FR-019)
* ``core.base.fallback`` — :func:`base_fallback_panel` (tier-8 universal)
...
from scistudio.panels.helpers import sanitize_svg
...
def _ref_for(request: PreviewRequest) -> StorageReference:
    ...
    :class:`~scistudio.panels.session.PreviewSessionManager` populates so
    providers need no catalog access — this is what a package panel reads.
...
def _error_envelope(request: PreviewRequest, code: object, message: str) -> PreviewEnvelope:
    from scistudio.panels.models import PreviewErrorCode, PreviewErrorInfo
...
def dataframe_panel(request: PreviewRequest) -> PreviewEnvelope:
...
def array_panel(request: PreviewRequest) -> PreviewEnvelope:
    ...
    # the raster fallback that the imaging package's Image panel reuses when
...
def series_panel(request: PreviewRequest) -> PreviewEnvelope:
...
def text_panel(request: PreviewRequest) -> PreviewEnvelope:
...
def artifact_panel(request: PreviewRequest) -> PreviewEnvelope:
...
def composite_panel(request: PreviewRequest) -> PreviewEnvelope:
...
def collection_panel(request: PreviewRequest) -> PreviewEnvelope:
...
def plot_panel(request: PreviewRequest) -> PreviewEnvelope:
...
def base_fallback_panel(request: PreviewRequest) -> PreviewEnvelope:
    """Tier-8 universal fallback: treat anything unknown as an artifact."""
    return artifact_panel(request)
...
def core_panel_specs() -> list[PanelSpec]:
    """Return the core fallback :class:`PanelSpec` list (FR-012).

    These are registered unconditionally by :meth:`PanelRegistry.load_core`.
    ...
    return [
        PanelSpec(
            ...
            features=("table", "page", "sort"),
            backend_provider=dataframe_panel,
        ),
        PanelSpec(
            ...
            owner_name="scistudio",
            target_type="Array",
            features=("shape", "slice", "matrix", "colormap"),
            backend_provider=array_panel,
        ),
        PanelSpec(
        ...
        PanelSpec(
            ...
            owner_name="scistudio",
            target_type="Text",
            features=("text", "truncate", "editor_handoff"),
            backend_provider=text_panel,
        ),
        PanelSpec(
            ...
            features=("download",),
            backend_provider=artifact_panel,
        ),
        PanelSpec(
            ...
            features=("slots", "child_routing"),
            backend_provider=composite_panel,
        ),
        PanelSpec(
            ...
            features=("count", "sample", "child_routing"),
            backend_provider=collection_panel,
        ),
        PanelSpec(
        ...
        PanelSpec(
            ...
            priority=-100,
            features=("download",),
            backend_provider=base_fallback_panel,
        ),
    ]
