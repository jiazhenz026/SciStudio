"""DataFrame preview paging + LRU cache (panels-owned).

ADR-048 / issue #1598: this table-paging cache is consumed only by the
panel subsystem (``panels.data_access``). It previously lived under
``scistudio.api.runtime._preview_cache``, which forced the lower panel
layer to import *up* into the API layer (an inverted dependency, unguarded by
the layer-dependency test). It now lives here so ``panels`` is
