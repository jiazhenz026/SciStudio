"""Raster preview encoding helpers (panels-owned).

ADR-048 / issue #1598: the raster load/downsample/encode pipeline used by the
panel subsystem previously lived under ``scistudio.api.runtime._preview_image``,
forcing the lower panel layer to import *up* into the API layer. It now lives
here so ``panels`` is self-contained; the API layer's legacy composite-preview
path imports these *down* from panels.
