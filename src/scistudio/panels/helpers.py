"""Public author-facing panel helpers.

This is the canonical home for the small, reusable helpers a package-owned
panel may import, alongside the two main author roots
:mod:`scistudio.panels.models` and :mod:`scistudio.panels.data_access`.

Today it exposes a single helper, :func:`sanitize_svg`, which a package SVG/plot
panel uses to scrub SVG text before returning it.
...
def sanitize_svg(svg_text: str) -> tuple[str, bool]:
    ...
    Call this before returning SVG from a panel to remove ``<script>``
