...
:class:`PreviewDataAccess` is the only surface a panel provider may use to
...
# :class:`~scistudio.panels.models.PreviewLimits` on ``request.limits``.
...
class ArrayPlane:
    ...
    plane — the panel renders these as a numeric heatmap table, not a lossy
...
class PreviewDataAccess:
    ...
    This is the only sanctioned way a panel provider reads a target's stored
    ...
    def dataframe_page(
        ...
        *,
        ...
        """
        from scistudio.panels._table_cache import _get_preview_table
    ...
    def png_data_uri(self, matrix: list[list[float | None]]) -> str:
        ...
        generated reference; new panels return the numeric ``matrix`` from
        ...
        """
        from scistudio.panels._raster import _image_data_uri_from_matrix
    ...
    def _downsample(self, matrix: Any) -> list[list[float | None]]:
        ...
        from scistudio.panels._raster import _downsample_matrix
...
# Public author surface for ``scistudio.panels.data_access`` (ADR-052 §8.2):
...
# ``request.limits`` / :class:`~scistudio.panels.models.PreviewLimits`) and
