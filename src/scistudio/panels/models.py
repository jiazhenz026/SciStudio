...
A *panel* turns a stored data object, collection, or artifact into a
bounded, JSON-safe view the frontend can display. This module is one of the
canonical roots you import from when you write a package-owned panel; the
others are :mod:`scistudio.panels.data_access` (the bounded reader injected
on each request) and :mod:`scistudio.panels.helpers` (``sanitize_svg``).
...
* :class:`PanelSpec` — a provider declaration: its id, provenance tier,
  target type, priority, features, backend provider, and frontend manifest.
* :class:`FrontendManifest` — the same-origin descriptor a panel ships for
...
* :class:`PreviewRequest` — the input a provider receives: the target, the
  ...
  :class:`~scistudio.panels.data_access.PreviewDataAccess`, the budgets, and
...
* The :data:`PreviewProvider` callable type and the :class:`PanelEntryPoint`
...
class EnvelopeKind(StrEnum):
    ...
    These are the panel-domain kinds. They are deliberately distinct from
...
class PreviewErrorCode(StrEnum):
    ...
    """Two panels tied for the target and neither could be preferred."""
    UNKNOWN_PREVIEWER = "unknown_previewer"
    """The requested panel id is not registered."""
    UNKNOWN_TARGET = "unknown_target"
    """No panel (not even a core fallback) matched the target."""
    MISSING_BUNDLE = "missing_bundle"
    """A panel declares a frontend manifest but no servable bundle."""
    PROVIDER_EXCEPTION = "provider_exception"
    """A backend provider raised while rendering the preview."""
    INVALID_SPEC = "invalid_spec"
    """A panel spec failed validation when it was registered."""
    DUPLICATE_PREVIEWER_ID = "duplicate_previewer_id"
    """Two specs declared the same panel id."""
...
class PreviewSource:
    ...
    "node X, output port Y" without making the panel part of the workflow.
...
@dataclass(frozen=True)
class PreviewTarget:
    """Identifies the thing a panel is asked to render.
    ...
    panel. You build a target to ask the preview system for a preview, and a
...
class FrontendManifest:
    """Describes the same-origin UI bundle a panel ships for the frontend.

    A package or project panel can include a small JavaScript module (and
    ...
    previewer_id: str
    """Stable id of the panel that owns this bundle."""
    module_url: str
    ...
    """Panel API version the bundle targets; must equal
    :data:`PANEL_API_VERSION` to mount without a diagnostic."""
...
class PanelSpec:
    ...
    which target type the panel handles, how strongly it should win, what
    features it offers, and which backend callable renders the envelope. You
    construct specs to register a panel; the router and session manager read
    them.
...
class PreviewMetadata:
    ...
    the user is looking at. ``extra`` carries panel-owned shape/type details.
    ...
    """Panel-owned extra metadata merged into the wire payload, e.g.
    ``{"shape": [...], "dtype": "..."}``."""
...
class PreviewEnvelope:
    ...
    a typed error. Build it with the panel id, the target, an
    ...
    previewer_id: str
    """Id of the panel that produced this envelope."""
    target: PreviewTarget
    """The normalized target that was previewed."""
    kind: EnvelopeKind
    """The display kind of ``payload``."""
    payload: dict[str, Any] = field(default_factory=dict)
    """Panel-owned, bounded, JSON-safe payload for the frontend."""
...
class PreviewRequest:
    ...
    spec: PanelSpec
    """The panel spec the router selected."""
    query: dict[str, Any]
...
@provisional(since="0.3.1")
@runtime_checkable
class PanelEntryPoint(Protocol):
    """The callable a package wires to the ``scistudio.previewers`` entry point.

    A package advertises its panels by pointing a ``scistudio.previewers``
    entry point at a zero-argument callable that returns a list of
    :class:`PanelSpec`. Each returned spec declares
    ...
    Example:
        ...
        where ``get_previewers() -> list[PanelSpec]``. An installed
    ...
    def __call__(self) -> list[PanelSpec]: ...
...
# without a tier badge; the provisional tier is recorded in the spec.
PanelSpecList = list[PanelSpec]
"""The accepted return type of a panel entry-point callable: a list of
:class:`PanelSpec`. Any other shape is rejected with a diagnostic."""
...
class RoutingAmbiguityError(PreviewError):
    """Two panels tie on tier, specificity, and priority (FR-004).
...
@internal()
class UnknownPanelError(PreviewError):
    """The requested panel id is not registered (runtime-raised; ADR-052 §8.1 Internal)."""
...
class UnknownTargetError(PreviewError):
    """No panel (not even a core fallback) matched the target (runtime-raised; ADR-052 §8.1 Internal)."""
...
class MissingBundleError(PreviewError):
    """A panel declares a frontend manifest but no servable bundle (runtime-raised; ADR-052 §8.1 Internal)."""
...
class InvalidSpecError(PreviewError):
    """A panel spec failed validation at registration time (runtime-raised; ADR-052 §8.1 Internal)."""

    code = PreviewErrorCode.INVALID_SPEC


@internal()
class DuplicatePanelIdError(PreviewError):
...
# Public author surface for ``scistudio.panels.models`` (ADR-052 §8.1).
...
# (``RoutingAmbiguityError``, ``UnknownPanelError``, ``UnknownTargetError``,
# ``MissingBundleError``, ``InvalidSpecError``, ``DuplicatePanelIdError``) —
