...
async def _status_rows() -> list[dict[str, Any]]:
    ...
    While the ``ai_chat_disabled`` capability refuses agent sessions, no agent
    binary is run (no ``--version``, no auth-status command). Each such
    provider is reported unavailable with ``disabled: true``.
    """
    # Development references: ADR-055 Spec 4 FR-006, #2322.
