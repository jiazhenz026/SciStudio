...
- `load(self, config: 'BlockConfig', output_dir: 'str' = '') -> 'DataObject | Collection'` — _unmarked — no runtime stability marker_ — Read the configured ``path`` and return the loaded object.
- `save(self, obj: 'DataObject | Collection', config: 'BlockConfig') -> 'None'` — _unmarked — no runtime stability marker_ — Always raises — a SimpleLoader cannot save.
