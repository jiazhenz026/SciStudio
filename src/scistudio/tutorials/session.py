...
class SessionRecord:
    ...
    branch on the product's state — so where the reader came from is recorded
    ...
    @property
    def cursor(self) -> int:
        """Where ``step_id`` sits on the trail, or ``-1`` when it is not on it.

        The last occurrence, because a driver may route through the same step
        twice and the reader is at the most recent one.
        """
        if self.step_id is None:
            return -1
        for index in range(len(self.visited_step_ids) - 1, -1, -1):
            if self.visited_step_ids[index] == self.step_id:
                return index
        return -1
    ...
    def revisiting(self) -> bool:
        """Whether the reader is behind the furthest step they have reached."""
        cursor = self.cursor
        return 0 <= cursor < len(self.visited_step_ids) - 1

    def entered(self, step_id: str) -> SessionRecord:
        """Return this record having just entered *step_id*, trail extended."""
        return replace(
            self,
            step_id=step_id,
            step_entered_at=_now_iso(),
            visited_step_ids=(*self.visited_step_ids, step_id),
        )

    def moved_to(self, index: int) -> SessionRecord:
    ...
    def as_json(self) -> dict[str, Any]:
        return {
            ...
            "visited_step_ids": list(self.visited_step_ids),
    ...
    def from_json(cls, raw: Mapping[str, Any]) -> SessionRecord | None:
        ...
        visited = raw.get("visited_step_ids")
        ...
        return cls(
            ...
            visited_step_ids=tuple(str(item) for item in visited) if isinstance(visited, list) else (),
            status=status,
...
class SessionView:
    ...
    error: str | None
    #: Whether there is an earlier step to return to (#2138). Session-level
    #: rather than a step-view field: it is a fact about the trail this session
    #: walked, not about the step, and FR-041 keeps the driver's field set
    #: closed to exactly what a driver reports.
    can_go_back: bool = False
    #: Whether the reader is behind the furthest step they reached (#2138).
    #:
    #: Reported because two things the client draws depend on it and cannot be
    #: derived from the step alone. A revisited step reads as satisfied whether
    #: or not its condition still holds, so "satisfied" stops meaning "you just
    #: did this" — and a surface that hides its pointer on a satisfied step
...
class TutorialRuntime:
    ...
    def continue_active(self) -> SessionView:
        ...
        if satisfied.revisiting:
            # Forward along the trail, not forward through the driver (#2138).
            # The step ahead has already been entered, and entering it again
            # would run its `write` and `replay` actions a second time.
            return self._reposition(state, satisfied.moved_to(satisfied.cursor + 1), driver, tutorial_dir)
    ...
    def back_active(self) -> SessionView:
        ...
        asking the driver to un-advance, which nothing can do. Nothing is
        entered, so no entry action runs a second time, and no progress is
        given back — a step the reader satisfied stays satisfied, which is what
        lets them walk forward again afterwards.
        ...
        if not record.can_go_back:
            return self._reevaluate(state, record, driver, tutorial_dir)
        return self._reposition(state, record.moved_to(record.cursor - 1), driver, tutorial_dir)
    ...
    def _reposition(
        ...
        reflect the world as it is now, and :meth:`_reevaluate` is what supplies
    ...
    def _reevaluate(
        ...
        try:
            ...
            return self._view(self._fail(state, record, self._driver_failure(record, exc)))
        if not satisfied and record.revisiting and record.step_id in record.satisfied_step_ids:
            # A step behind the reader that they have already cleared reads as
            # cleared, whatever the condition says now (#2138). Going back is
            # for re-reading, and a `since_step_entry` term — or a workflow the
            # reader has moved on from — would otherwise answer no and leave
            # them with a dark Continue on a step they already finished. Only
            # behind the furthest point: on the live step the judgment is the
            # judgment, and FR-054a's honesty depends on it.
            satisfied = True
        if satisfied:
