...
"""The joint block: two modalities in, one labeled table out.
...
analyzed against item *i* of ``expression``, and nothing in this block can
...
def _initial_centres(points: np.ndarray, k: int) -> np.ndarray:
    """Choose *k* starting centers, deterministically.
    ...
    Same intent — spread the starting centers out — with no randomness to
    record, which is what a result a reader has to trust wants.
    """
    center = points[np.argmin(((points - points.mean(axis=0)) ** 2).sum(axis=1))]
    centers = [center]
    while len(centers) < k:
        distances = np.min(
            np.stack([((points - chosen) ** 2).sum(axis=1) for chosen in centers]),
            axis=0,
        )
...
def kmeans(points: np.ndarray, k: int) -> np.ndarray:
    ...
    centers = _initial_centres(points, k)
    assignment = np.full(len(points), -1)
    for _ in range(MAX_ITERATIONS):
        distances = np.stack([((points - center) ** 2).sum(axis=1) for center in centers], axis=1)
...
def _order_clusters(points: np.ndarray, assignment: np.ndarray) -> np.ndarray:
    """Renumber clusters by their center's total signal, largest first.
    ...
    a different machine, and the figure's colors move for no reason.
    """
    order = sorted(
...
class JointRegionProfilesBlock(ProcessBlock):
    ...
    input_ports: ClassVar[list[InputPort]] = [
        ...
        InputPort(name="expression", accepted_types=[DataFrame], description="One normalized count table per section"),
    ]
    ...
    def run(self, inputs: dict[str, Any], config: BlockConfig) -> dict[str, Collection]:
        ...
        if not rows:
            raise ValueError("No position fell inside a labeled region; check the pairing and the segmentation.")
        ...
        # Standardize each gene across regions before clustering. Without it a
        ...
        standardized = centered / np.where(spread > 0, spread, 1.0)

        assignment = _order_clusters(standardized, kmeans(standardized, int(config.get("clusters", 3))))
...
def _region_rows(label_image: Image, table: DataFrame) -> list[dict[str, Any]]:
    """One row per labeled region of a single section.

    ``Segment Cells`` is tutorial 2's block, reused here, and it emits two
    channels on one grid — the micrograph it read on ``c=0``, the labels it
    found on ``c=1`` — so that its preview can draw one over the other. This
    wants the labels. A plain 2-D map is still read as-is, so a label map from
    anywhere else profiles the same way.
    """
