...
button on the "normalize the expression" step.

Raw counts are not comparable between positions, because positions do not
capture the same amount of material. Normalization is the correction for that,
...
# — a normalization that refuses is a workflow that stops — it goes ahead on
...
class NormalizeExpressionBlock(ProcessBlock):
    ...
    def process_item(self, item: DataFrame, config: BlockConfig, state: Any = None) -> DataFrame:
        """Normalize one section's count table.
        ...
        Returns:
            The same table with every gene column normalized and ``log1p``-ed.
        """
