...
One panel per section, region centroids in image coordinates, color by
cluster. Read it the way you would read the slide: if the clustering found
tissue programs rather than sequencing depth, each section carries a
comparable spread of every color, because the same kinds of tissue recur
section to section. A section that turns one color is the picture of a
clustering that grouped by batch.
"""


def render(collection):
    """One panel per section: region centroids, colored by cluster."""
