...
Core tutorial 4 lands this block when it creates the project. It is ordinary
...
Both methods genuinely run, and neither is perfect — which is the lesson.
``threshold`` takes one cut for the whole frame, so it finds the obvious
foreground and whatever debris shares its brightness. ``adaptive`` chases
*local* contrast, and on a frame that is mostly background that promotes every
noise grain to an object of its own.
