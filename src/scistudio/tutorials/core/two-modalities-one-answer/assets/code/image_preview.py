...
when it creates the project. Without it an ``Image`` has no previewer of its
own, so the preview panel walks up the type chain, finds the core ``Array``
previewer, and shows a section as a table of numbers. That fallback is correct
— an Image *is* an Array — it is just not how anyone wants to look at one.
