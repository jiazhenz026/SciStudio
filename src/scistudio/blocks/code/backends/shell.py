...
def _environment_delta(context: CodeBlockRuntimeContext) -> dict[str, str]:
    ...
    env_delta.update(
        {
            "SCISTUDIO_CODEBLOCK_EXCHANGE_DIR": str(exchange_dir),
            "SCISTUDIO_CODEBLOCK_INPUTS_DIR": str((exchange_dir / "inputs").resolve()),
            "SCISTUDIO_CODEBLOCK_OUTPUTS_DIR": str((exchange_dir / "outputs").resolve()),
            "SCISTUDIO_CODEBLOCK_PROJECT_DIR": str(context.project_dir.resolve()),
            "SCISTUDIO_CODEBLOCK_SCRIPT_PATH": str(context.script_path.resolve()),
        }
    )
