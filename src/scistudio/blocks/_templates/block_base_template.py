"""Starter template for a custom SciStudio block (per ADR-036 §3.12).
...
``from scistudio.blocks.base import Block, BlockSpec, PortSpec``. In the
current codebase, ``BlockSpec`` lives in ``scistudio.blocks.registry`` and
