...
_DEPRECATION_MESSAGE = (
    "scistudio.core.metadata_store is deprecated and superseded by ADR-038's "
    "unified lineage store (scistudio.core.lineage.LineageStore). The "
    "MetadataStore shim is retained for the 6-month deprecation window; "
    "new code MUST use the unified store via scistudio.core.lineage."
)
...
class MetadataStore:
    ...
    its own. The unified store lives at ``<project>/.scistudio/lineage.db``
...
def get_metadata_store() -> MetadataStore | None:
    ...
    all reads delegate to :class:`~scistudio.core.lineage.LineageStore`. Use
