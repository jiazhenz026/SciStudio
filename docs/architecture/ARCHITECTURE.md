...
for any workflow whose retained run recorded no artifact paths — each of those
states would otherwise read as "everything is reclaimable". `scistudio storage
gc` performs the same sweep on demand and previews unless given `--apply`;
`scistudio storage usage` reports per-workflow artifact size. Setting
`SCISTUDIO_ARTIFACT_RETENTION=off` disables the automatic sweep and leaves the
