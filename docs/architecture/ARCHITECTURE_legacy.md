...
> `docs/architecture/ARCHITECTURE.md`. This legacy file is not used as the
> current ADR-042 architecture drift target.
...
- **`LoadData`** at `src/scistudio/blocks/io/loaders/load_data.py` — input-only. Implements `load()` by dispatching to one of six private module-level `_load_*` functions selected by the `core_type` config enum (ADR-028 Addendum 1 §C9). Module-level helpers were extracted to private sibling modules in #1459 (Phase 2 of #1427): `loaders/_capability.py` (FormatCapability declarations + format dispatch) and `loaders/_helpers.py` (path + pickle helpers, MIME/text format lookups). The siblings hold module-level functions only — §C9 ("private functions, not helper classes") is satisfied verbatim.
- **`SaveData`** at `src/scistudio/blocks/io/savers/save_data.py` — output-only. Mirrors `LoadData` with six private `_save_*` dispatch functions. Same Path-D sibling layout: `savers/_capability.py`, `savers/_helpers.py`, plus `savers/_streaming.py` for the ADR-031 Phase 3 streaming export paths (zarr-to-zarr copy + CSV/Parquet row-group writers).
...
| **Format capability** | A stable IOBlock boundary declaration that converts between an external format and a DataObject type. Capability IDs are workflow replay keys. |
