...
Reusable patterns for driving the SciEasy GUI via Chrome MCP. Each
...
`localStorage["scieasy-studio-ui"].state.tabs` is the canonical persistence
...
```javascript
const state = JSON.parse(localStorage.getItem('scieasy-studio-ui')).state;
JSON.stringify({
...
```powershell
# 1. Kill the existing scieasy gui process
Get-Process python | Where-Object { $_.CommandLine -match 'scieasy gui' } | Stop-Process -Force

# 2. Wait for the port to free
Start-Sleep 1

# 3. Restart with the same args
Start-Process -NoNewWindow scieasy -ArgumentList 'gui', '--port', '8000', '--no-browser'
...
If you are running from a worktree, double-check `scieasy.__file__` —
