...
Backend (`scieasy gui` / `scieasy serve`) and frontend (`vite`) lifecycle
...
# SciEasy backend processes
Get-CimInstance Win32_Process -Filter "Name='python.exe'" |
  Where-Object { $_.CommandLine -like '*scieasy gui*' -or $_.CommandLine -like '*scieasy serve*' } |
  Select-Object ProcessId, CommandLine
```
...
**Prebuilt SPA via `scieasy gui`** (most scenarios):
...
**Vite dev server + `scieasy serve`** (frontend-dev scenarios only):

```powershell
# Backend headless
Start-Process -NoNewWindow scieasy -ArgumentList 'serve', '--port', '8000'
...
If running from a worktree, verify which `scieasy` is actually getting
imported:
...
If the path is not under the current worktree's `src/scieasy/`, a prior
...
before launching, or use `python -m scieasy.cli.main gui ...` with
`PYTHONPATH=src` set.
...
- Always tag the process for cleanup — when you start it, capture the
  ...
  ```powershell
  $proc = Start-Process scieasy -ArgumentList '...' -PassThru
