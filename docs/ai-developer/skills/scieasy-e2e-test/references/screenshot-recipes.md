...
not DOM — so a hidden SciEasy tab gives you whichever tab IS active in
...
"@
$chrome = Get-Process chrome | Where-Object MainWindowTitle -Match 'SciEasy' | Select-Object -First 1
[TF]::SwitchToThisWindow($chrome.MainWindowHandle, $true)
Start-Sleep -Milliseconds 300

# If SciEasy is not the active tab, Ctrl+<N> selects it (N depends on tab strip order)
...
$step = "<step-N>"
$path = "C:\Users\jiazh\Downloads\scieasy-e2e-$sessionId-$step.png"
...
```
SendUserFile(
  files=["C:/Users/jiazh/Downloads/scieasy-e2e-<session-id>-step<N>.png"],
  status="proactive",
...
`scieasy-e2e-<session-id>-step<N>.png` where `<session-id>` matches the
frontmatter and `<N>` is the step number. Examples:

- `scieasy-e2e-2026-05-20-pr-1300-step3.png`
- `scieasy-e2e-2026-05-20-hotfix-869-launch.png`
- `scieasy-e2e-2026-05-20-hotfix-869-after-fix.png`
...
```powershell
Remove-Item C:\Users\jiazh\Downloads\scieasy-e2e-<session-id>-*.png
```
