...
description: Drive a live end-to-end SciEasy session — start the backend (and Vite if needed), open Chrome via Chrome MCP, walk the scenario file the user points at, capture screenshots/console/network as evidence, and write a pass/fail verdict back into that file. Trigger whenever the user says "跑 e2e", "do an e2e", "run e2e on PR #N", "verify in Chrome", "hotfix repro", or otherwise asks you to take a SciEasy change for a real spin in the browser instead of trusting CI alone. Also trigger when the user hands you a path under docs/ai-developer/e2e/.
metadata:
  type: scieasy-skill
...
# Skill — SciEasy E2E Test

## 1. What This Skill Is For

This skill turns a filled e2e scenario file into a real, observed run of
SciEasy: backend up, browser open, every step exercised, every regression
...
- No stale Vite / `scieasy gui` process is squatting on the port from
...
4. Capture the artifacts named in **Capture**. Screenshots use the
   ...
   `C:\Users\jiazh\Downloads\scieasy-e2e-<session-id>-step<N>.png` so
