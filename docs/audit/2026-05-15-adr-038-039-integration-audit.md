...
**Verdict: pass-with-fixes** — the integration is feasible and the two known hazards (H-A1, H-D1) are well-anticipated. **Three new P1 findings** require source edits at integration time, all in `src/scieasy/api/runtime.py` and one in `src/scieasy/core/lineage/store.py`. **Six P2** and **five P3** findings are documented for the integration / Phase 4 fix PR to decide upon.
...
**File:** `src/scieasy/api/runtime.py` (integration state; lines ~1476–1564 on the 039 track / 1240–1331 on the 038 track once merged)

**What the audit found:**

- D38-3.2 (PR #967) added `LineageStore.set_pending_git_commit(workflow_id, sha)` at `src/scieasy/core/lineage/store.py:271` ([verified by `git show origin/track/adr-038/lineage-db:src/scieasy/core/lineage/store.py | sed -n '271,304p'`]). It executes `UPDATE runs SET workflow_git_commit = ? WHERE run_id = (SELECT run_id FROM runs WHERE workflow_id = ? ORDER BY started_at DESC LIMIT 1)`.
- D39-2.5 (PR #959) added the `set_pending_git_commit` **call** inside `ApiRuntime.start_workflow` ([`src/scieasy/api/runtime.py:1554-1564` on track/adr-039](https://github.com/zjzcpj/SciEasy/blob/track/adr-039/git-versioning/src/scieasy/api/runtime.py#L1554-L1564)). On the 039 track alone this is a no-op (no `lineage_store` exists) — D39-2.5's own test asserts the no-op path.
...
Better: thread the SHA through `_build_lineage_recorder`'s `RunRecord(workflow_git_commit=...)` constructor so the row gets the field at INSERT time. `RunRecord` already has `workflow_git_commit: str | None` (`src/scieasy/core/lineage/record.py:36`), so add a single keyword argument and replace `set_pending_git_commit` with an in-insert write. This eliminates the cross-table UPDATE entirely.

### P1-2 — `runs.workflow_dirty` is never populated despite the 039 code claiming it's "the source of truth"

**File:** `src/scieasy/api/runtime.py` (integration `start_workflow`, lines ~1500-1515 of the 039 source) and `src/scieasy/core/lineage/record.py:41` / `src/scieasy/core/lineage/store.py:64`.

**What the audit found:**

- `LineageStore.runs.workflow_dirty INTEGER NOT NULL` is part of the ADR-038 schema (`src/scieasy/core/lineage/store.py:64`).
- `RunRecord.workflow_dirty: int = 0` defaults to 0 on every insert (`src/scieasy/core/lineage/record.py:41`).
- `_build_lineage_recorder` never threads any value into this field (`src/scieasy/api/runtime.py:1347-1357` on the 038 track).
...
**Files:** `src/scieasy/ai/agent/system_prompt.py` (no occurrences of "agent:" prefix); `src/scieasy/blocks/ai/ai_block.py:34-36` (docstring acknowledges no enforcement)

**What the audit found:**

- ADR-039 §3.4a defines `agent:` as the commit prefix for AI-authored commits ([line 197](https://github.com/zjzcpj/SciEasy/blob/main/docs/adr/ADR-039.md#L197)).
...
- Grep across `src/scieasy/ai/agent/` for `"agent:"` returns **zero matches**. The system prompt at `src/scieasy/ai/agent/system_prompt.py` does not mention the prefix convention at all.
...
**Recommended fix (post-integration, not in Phase 4 fix PR):** Update `src/scieasy/ai/agent/system_prompt.py` to instruct the agent to use `git commit -m "agent: ..."` for any commit it authors. File a tracking issue.

### Section A items verified clean

- **`git.head_changed` reaches the UI**: `src/scieasy/api/routes/workflow_watcher.py:304` (`_GitHeadHandler` class) emits the event; `src/scieasy/api/ws.py` forwards it; `frontend/src/hooks/useWebSocket.ts:147-166` consumes it and calls `useAppStore.getState().invalidateHistory()`. (See P3-2 for the lineage side.)
- **Dual watcher collapse (D39-3.2)**: `src/scieasy/api/routes/workflow_watcher.py` correctly attaches `.git/` as a second `Observer.schedule()` inside `WorkflowWatcher.start_for_project`. The collapse from the previously-separate `core.versioning.watcher.GitChangeWatcher` (asyncio-poll) into the unified watchdog Observer is sound; verified at `app.py:84-94` post-merge.
...
**File:** `src/scieasy/api/runtime.py:793-822` (conflict region 2 of 3 in `delete_project`)
...
- A naive "take 039" closes the dead-code MetadataStore shim (harmless) but leaves the live `lineage_store` SQLite handle open, and `shutil.rmtree` (or even `_rmtree_force` calling `shutil.rmtree` internally) refuses to delete the locked `.scieasy/lineage.db` on Windows.
...
**Recommended fix:** Suppress the deprecation warning inside `src/scieasy/ai/agent/mcp/tools_inspection.py` and `tools_workflow.py` callers via `warnings.filterwarnings("once", category=DeprecationWarning, module="scieasy.core.metadata_store")` at import boundary. Or migrate the MCP tools to import from `scieasy.core.lineage.store` directly.

### Section C items verified clean

- **`lineage.db.runs` schema**: confirmed at `src/scieasy/core/lineage/store.py:57-79`; includes `workflow_git_commit TEXT`, `workflow_dirty INTEGER NOT NULL`, `parent_run_id TEXT REFERENCES runs(run_id)`, `execute_from_block_id TEXT`.
- **`block_version` force-injection**: `src/scieasy/blocks/registry.py` post-D38-2.2 calls `importlib.metadata.packages_distributions()` and stamps `scieasy.__version__` for in-tree blocks. D38-3.2 (#967) added `BlockRegistrationError` so unknown distributions raise per ADR §3.3 instead of returning literal `"unknown"`. The recent fix `3cf254f` ("scieasy_blocks_* monorepo packages fall back to scieasy version") confirms `scieasy_blocks_imaging|lcms|srs` are covered.
...
- `src/scieasy/api/app.py` lifespan — verified: `runtime = ApiRuntime()` initializes both lineage store (`_init_lineage_store` chain) and git (via `open_project` → auto-init); the `WorkflowWatcher` schedule attaches both project workflows dir AND `.git/` (the D39-3.2 collapse).
...
- **Verified**: `LineageStore.set_pending_git_commit` method exists on track/adr-038 at `src/scieasy/core/lineage/store.py:271`. Implementation: SELECT most-recent run, UPDATE its `workflow_git_commit`. Supports `sha=None` to clear.
...
- Phase 4a (a) — `sqlite3 .scieasy/lineage.db "SELECT run_id, workflow_id, workflow_git_commit, workflow_dirty FROM runs"` should produce non-NULL SHAs aligned to the correct run (per P1-1 fix). Compare to `git rev-parse HEAD` immediately after each `start_workflow`.
