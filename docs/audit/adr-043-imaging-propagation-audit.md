...
| `CellposeSegment` | `segmentation/cellpose_segment.py` | **C** | **carry** | **Updated in this PR** twice: (1) the `mask_img` rebuilt `Image.Meta` carries `ome=image.meta.ome`; (2) the `Label.Meta` constructed in `process_item` carries `ome=item.meta.ome`. Output Label raster is shape-aligned with source Image (y, x). Tests: implicit via the same propagation contract; CellposeSegment requires the `[cellpose]` extra so a dedicated runtime test would gate on that — propagation pattern is identical to BlobDetect / Watershed and is verified by static inspection. |
