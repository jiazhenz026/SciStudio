...
- §3.4a "Agent commit prefix" — only the `git log --format` plumbing knows about `agent:`; **no in-tree code path actually emits an `agent:` commit**. AIBlock docstring claims `mcp__scistudio__git_commit` does, but that MCP tool does not exist in the repo. See P2-A.
...
- ruff: **pass** (`ruff check src/scistudio/core/versioning/ src/scistudio/api/routes/git.py` → "All checks passed")
...
- backend boot smoke: **pass** (`scistudio serve --port 8123` answers `/health` → 200 and `/api/git/status` → 409 "No active project" as expected for a no-project boot)
...
**Affected files**:
- `src/scistudio/api/app.py:84-112` (constructs `core.versioning.watcher.GitChangeWatcher`)
- `src/scistudio/core/versioning/watcher.py:153-167` (emits `git.head_changed` with key `head_sha`)
- `src/scistudio/api/routes/workflow_watcher.py:427-453` (`_GitHeadHandler.on_any_event` emits `git.head_changed` with key `commit_sha`)
...
2. Delete `src/scistudio/core/versioning/watcher.py` (or downgrade it to a `from .._compat import ...` shim) + remove the construction in `app.py:84-112`.
...
**Affected files**:
- `src/scistudio/api/runtime.py:1333-1343` (calls `lineage_store.set_pending_git_commit(...)` defensively)
- `src/scistudio/core/lineage/store.py` on `track/adr-038/lineage-db` (the method does NOT exist)
...
**Affected file**: `src/scistudio/blocks/ai/ai_block.py:11-32` (claims two emission layers)

**Symptom**: The AIBlock docstring states the `agent:` prefix is enforced via (1) `scistudio.api.routes.mcp_tools.git_commit` passing `prefix="agent"` to `GitEngine.commit()`, and (2) the agent's system-prompt being told the convention. Layer (1) does **not exist in the repo** — `grep -rn "mcp_tools" src/scistudio/` returns no matches, and there is no `git_commit` MCP tool registered anywhere under `src/scistudio/ai/`. The convention is therefore documented but unenforced: the only way a real `agent:` commit appears in `git log` is if the agent itself types `git commit -m "agent: ..."` in the PTY, relying entirely on the system-prompt rule. The History "Manual milestones" filter (default) then works as long as agents obey the prompt — but Phase 4b Test 2 ("Open AI chat; instruct agent to make a change and commit it") will likely fail the "shows `agent:` prefix + 🤖 icon" criterion unless the system prompt is verified.

**Recommended fix**:
1. Either implement the `mcp__scistudio__git_commit` MCP tool (a thin wrapper around `GitEngine.commit(prefix="agent")`) and register it on the MCP server, **or**
...
**Affected file**: `src/scistudio/api/app.py:84-112`
...
**Affected file**: `src/scistudio/api/runtime.py:1255-1294`
...
**Severity**: P2 — corner case (project created by a non-SciStudio path), but it's the failure mode users blame on "SciStudio lost my history".

### P3-A — `is_repository` may return false negative on `.git` worktree-link files

**Affected file**: `src/scistudio/core/versioning/git_engine.py:171-183`
...
**Affected file**: `src/scistudio/core/versioning/git_engine.py:553-563`
...
**Affected file**: `src/scistudio/core/versioning/git_engine.py:283-307`
...
3. **P2-A**: decide between (a) implementing the missing `mcp__scistudio__git_commit` tool and (b) correcting the AIBlock docstring + adding the system-prompt verification to Phase 4b.
