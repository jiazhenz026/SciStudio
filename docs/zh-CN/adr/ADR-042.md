...
- The `Assisted-by:` git trailer (§13) uses an agent-agnostic format
  `<x id="56" data-kind="html-tag"/>:<x id="57" data-kind="html-tag"/>` and CI validates only format compliance, never
...
1. **`.workflow/ci/ratchet.py`** — per-tool wrapper: reads previous
   finding count from `docs/audit/baselines/<x id="58" data-kind="html-tag"/>.json`, compares to
...
**Phase-1 verification artifact**: `docs/audit/reports/<x id="59" data-kind="html-tag"/>/ci-implementability.json`
...
RepoRelativePath = Annotated[str, Field(min_length=1, pattern=r"^[^/](<x id="0" data-kind="md-url"/>)?$")]
...
| `assisted_by` | list[str] | yes (may be empty) | Format `<x id="60" data-kind="html-tag"/>:<x id="61" data-kind="html-tag"/> [tools]` |
...
2. Mutates ADR-N's frontmatter: sets `status: Superseded`, `superseded_by: <x id="62" data-kind="html-tag"/>`,
   `date_superseded: <x id="63" data-kind="html-tag"/>`.
...
- Specs identify by `spec_id: <x id="64" data-kind="html-tag"/>` rather than integer.
...
Reports are stored under `docs/audit/reports/YYYY-MM-DD/<x id="65" data-kind="html-tag"/>.json`,
...
The how-to lives in `.claude/skills/<x id="66" data-kind="html-tag"/>/SKILL.md` (and equivalent paths
...
|---|---|---|
| `Signed-off-by:` | `Name <x id="67" data-kind="html-tag"/>` | Always (DCO-equivalent attestation) |
| `Assisted-by:` | `<x id="68" data-kind="html-tag"/>:<x id="69" data-kind="html-tag"/> [tools]` | Every agent-authored commit (§13.2) |
| `Fixes:` | `<x id="70" data-kind="html-tag"/> ("subject")` | Every bug-fix commit |
| `ADR:` | `ADR-NNN` | Every commit implementing or modifying an ADR |
| `Reviewed-by:` | `Name <x id="71" data-kind="html-tag"/>` | When the commit has been reviewed by another human |
| `Co-authored-by:` | `Name <x id="72" data-kind="html-tag"/>` | When multiple authors contributed |
| `Reviewed-locally:` | `<x id="73" data-kind="html-tag"/>` | When skipping Codex review (Tier 2 humans only, §25) |
| `Maintainer-Override:` | `<x id="74" data-kind="html-tag"/>` | When a Tier 2 human bypasses a check (§25) |
| `Human-Override:` | `<x id="75" data-kind="html-tag"/>` | When a Tier 1 human exercises a documented exemption (§25) |

### 13.2 `Assisted-by:` agent-equality format

The format `<x id="76" data-kind="html-tag"/>:<x id="77" data-kind="html-tag"/> [tools]` is agent-agnostic. Valid examples:
...
1. `pre-commit run --files <x id="78" data-kind="html-tag"/>` (fast subset relevant to changed
   files).
...
|---|---|---|---|---|---|
| Claude Code | `_skills/scieasy/<x id="79" data-kind="html-tag"/>/SKILL.md` | `.claude/skills/<x id="80" data-kind="html-tag"/>/SKILL.md` (flat per ADR-040 Add.1) | `~/.claude/skills/<x id="81" data-kind="html-tag"/>/SKILL.md` | Skill registry does not recurse; paths must be flat (one level under `skills/`) | ADR-040 §3.4 + Add.1 |
| Codex | (same `_skills/`) | `.codex/skills/<x id="82" data-kind="html-tag"/>/SKILL.md` AND `.agents/skills/<x id="83" data-kind="html-tag"/>/SKILL.md` (flat) | `~/.codex/skills/<x id="84" data-kind="html-tag"/>/SKILL.md` | Same flat-only constraint | ADR-040 §3.7 + §3.9 + Add.1 |
| Cursor | (same `_skills/`) | `.cursor/rules/<x id="85" data-kind="html-tag"/>.md` | n/a (no user-global concept) | Cursor uses single-file `.md` rules, not directories | (deferred ADR) |
| Aider | (same `_skills/`) | `.aider.skills/<x id="86" data-kind="html-tag"/>/SKILL.md` | `~/.aider.skills/<x id="87" data-kind="html-tag"/>/SKILL.md` | Convention; Aider has no official "skills" concept | (deferred ADR) |
| Gemini | (same `_skills/`) | `.gemini/skills/<x id="88" data-kind="html-tag"/>/SKILL.md` | `~/.gemini/skills/<x id="89" data-kind="html-tag"/>/SKILL.md` | Convention | (deferred ADR) |

`agent_provisioning` (§17.3) is the single source-of-truth installer that
materializes a skill from `_skills/scieasy/<x id="90" data-kind="html-tag"/>/` into every runtime's
...
- Pass `scieasy-skill-creator validate <x id="92" data-kind="html-tag"/>` before commit.
...
| 4 | `branch` | Branch from latest `main`; name `<x id="93" data-kind="html-tag"/>/issue-<x id="94" data-kind="html-tag"/>/<x id="95" data-kind="html-tag"/>/<x id="96" data-kind="html-tag"/>` | `git fetch origin && git checkout -b ...` |
...
> `tools/codemods/adr-NNN-<x id="97" data-kind="html-tag"/>.py`. CI verifies that applying the codemod
...
- Each `docs/zh-CN/X.md` carries frontmatter `source_sha: <x id="98" data-kind="html-tag"/>`.
...
|---|---|---|
| ` <x id="44" data-kind="code-fence"/>python\n# pytest-examples: skip ` | Pseudocode, design sketches, or examples that intentionally raise `NotImplementedError` | Discovered + explicitly skipped (still parsed by syntax check) |
| ` <x id="45" data-kind="code-fence"/>bash ` / ` <x id="46" data-kind="code-fence"/>toml ` / ` <x id="47" data-kind="code-fence"/>jinja ` etc. | Shell, config, structured-data snippets | NOT discovered |
...
| Pre-push | Full audit (doc_drift, fact_drift, closure, frontmatter_lint, trailer_lint, skill-installation-check) | `docs/audit/reports/local/<x id="99" data-kind="html-tag"/>.json` (gitignored locally) |
...
A Tier 1 or Tier 2 human may declare `--trivial "<x id="100" data-kind="html-tag"/>"` on a commit when:
...
| 1 | @claude (parallel via agent-manager) + @jiazhenz026 | @jiazhenz026 | First full audit report at `docs/audit/reports/<x id="101" data-kind="html-tag"/>/full.json` | Reduced-scope sub-track sign-off |
...
gate) AND first full audit report at `docs/audit/reports/<x id="104" data-kind="html-tag"/>/full.json`
...
| Element | Value |
|---|---|
| Required artifact | Baseline full-audit report at `docs/audit/reports/<x id="105" data-kind="html-tag"/>/full.json` |
...
`docs/audit/reports/<x id="106" data-kind="html-tag"/>/ci-implementability.json` confirms:
...
**Baseline storage**: `docs/audit/baselines/<x id="107" data-kind="html-tag"/>.json` — committed file
...
`# noqa: <x id="108" data-kind="html-tag"/>(#<x id="109" data-kind="html-tag"/>)` allowed only with:
...
2. **Section + sub-element match**: prefix `ADR-NNN §X[.Y]` followed by
   ` (component <x id="110" data-kind="html-tag"/>)` where `<x id="111" data-kind="html-tag"/>` is a specific cell, table-row, or
...
Findings are written to `docs/audit/adr-self-audit/<x id="112" data-kind="html-tag"/>-<x id="113" data-kind="html-tag"/>.json`. CI
...
1. For each ADR/spec lacking frontmatter, inserts a stub based on inline
   metadata heuristics.
2. Marks fields the heuristic cannot infer as `# TODO(#<x id="115" data-kind="html-tag"/>): backfill X`.
