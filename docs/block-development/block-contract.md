...
summary: "Formal block authoring contract: Block ABC hierarchy, ports (concrete accepted types by default; no produced_type), run contract, ProcessBlock/IOBlock hooks, CodeBlock and SubWorkflow, IO format capabilities, dynamic ports, and config schema."
---
...
> **`OutputPort` has no `produced_type` field.** Both `InputPort` and
> `OutputPort` declare types through `accepted_types: list[type]` only. Older
> material that passed a `produced_type` keyword to `OutputPort` is invalid and
