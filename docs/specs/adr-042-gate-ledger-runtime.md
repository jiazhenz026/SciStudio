...
| `gate_record/labels.py` | Single label vocabulary (`admin-approved:bypass`, `admin-approved:core-change`, `admin-approved:merge`, `admin-approved:architecture-doc`, `admin-approved:agent-docs`, `human-authored`) and the bypass/authorization semantics. | **port** of the shared constants currently exported from `human_bypass_guard`. |
...
| `agent_docs_guard` | protected-agent-docs surface, `observed_admin_labels`, `requested_admin_labels`, pr_context | Change to `src/scistudio/_skills/**`, `src/scistudio/_agent_reference/**`, or `src/scistudio/agent_provisioning/templates/claude_agents_md.md` lacks `admin-approved:agent-docs` provenance or an administrator approval review (#2438). |
...
| `.pre-commit-config.yaml` `scistudio-gate-record-commit-msg` | Thin entry calling the `gate_record commit-msg` alias (which delegates to `check --mode commit-msg <message-file>`). The hook MUST use the alias, not the bare `check --mode commit-msg`: on the commit-msg stage pre-commit appends the message-file path, and the `check` subparser rejects that positional with exit 2, failing every commit (issue #1609 Defect 4). |
