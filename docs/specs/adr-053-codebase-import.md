---
spec_id: adr-053-codebase-import
title: "ADR-053 Codebase Import — A Guided Agent Session For Bringing Existing Code In"
status: Draft
...
scope:
  in:
    ...
    - A toolbar entry for import, enabled when a project is open and disabled otherwise.
...
governs:
  ...
  entry_points: []
  files:
    - docs/specs/adr-053-codebase-import.md
    - docs/adr/ADR-053.md
  excludes:
    - docs/user/reference/**
    - docs/user/llms.txt
...
# ADR-053 Codebase Import — A Guided Agent Session For Bringing Existing Code In

## 1. Change Summary

A lab that already owns years of analysis code is the user with the most to gain
from SciStudio and the highest cost of entry. ADR-053 §4 makes carrying that code
across a first-class path rather than asking them to re-author it.
...
What replaces them is smaller. Import is **a permanently available toolbar entry
that spawns a preconfigured agent session**. The product supplies the entry
point, a short dialog of framing questions, and the system prompt. The agent
supplies the work: it reads the codebase, proposes what could become blocks, asks
the user whatever it needs, writes the blocks, asks for data, writes a test that
checks the transcription preserves the original behavior, saves it in the
project, runs it, and reports.
...
path plus a fixed set of framing answers before any session is spawned. The
questions are recorded in §8; they are not yet decided.
...
**FR-006.** Starting the session MUST spawn a chat session using the same
mechanism the agent block uses. Import does not introduce a second way to run an
agent.

**FR-007.** The source path and every dialog answer MUST be composed into the
...
**FR-008.** The system prompt MUST instruct the agent to ask the user when it
...
**FR-009.** The session MUST be an ordinary chat session once started — the user
...
**FR-010.** Availability MUST resolve to one of four states, each with its own
guidance:
...
**FR-011.** The probe MUST build on the provider registry and
...
**FR-012.** Distinguishing `call_failed` from `ready` MUST require a **live
...
**FR-013.** `call_failed` MUST report the underlying cause and MUST NOT suggest
...
**FR-014.** The probe MUST NOT block the import dialog from rendering. A slow or
hanging provider MUST degrade to a reported state, never to a stuck surface.

**FR-015.** Availability MUST be consumable by any agent-dependent surface, not
only import. ADR-053 §5.2 names the Learning Center agent-setup entry as another
consumer.

## 6. What The Product Says About Correctness

**FR-016.** The import surface MUST state, before the session starts, that the
...
**FR-017.** This statement MUST NOT be collapsed into a dismissible notice or
...
**FR-018.** The product MUST NOT enforce the existence of a test, and MUST NOT
...
**FR-019.** The prompt MUST instruct the agent to:
...
**FR-020.** The prompt MUST make step 8 an expectation rather than an option. An
...
**FR-021.** Tests MUST be saved inside the project directory. The value of the
test is that it can be rerun by hand later, which requires the user to be able to
find it.

## 8. Open Questions

**OQ-1.** *What the dialog asks.* The fixed question set is not decided. It
determines what context the agent starts with and is the cheapest place to
improve session quality, since every answer costs the user one interaction and
saves the agent one round trip.
...
**OQ-3.** *Where tests are saved within the project.* FR-021 requires the project
...
not merged. If import is implemented first, FR-011 has nothing to consume.
...
match, and #2001's static-scan scope no longer exists.
...
| Prompt composition | Every dialog answer and the source path appear in the composed system prompt (FR-007) |
| Caveat presence | The caveat copy is present and the session cannot be started without it having been shown (FR-016, FR-017) |
| Availability states | All four states resolve correctly, including authenticated-but-failing (FR-010, FR-012) |
| Availability guidance | `call_failed` reports its cause and does not suggest reinstalling (FR-013) |
| Probe non-blocking | A hanging provider yields a reported state rather than a stuck dialog (FR-014) |
| No enforcement | Nothing blocks a block on the absence of a test file (FR-018) |
...
| Agent session spawn path | modify | Accept a composed system prompt (FR-006, FR-007) |
...
**The caveat can be ignored.** FR-016 puts it where the user starts, which is the
