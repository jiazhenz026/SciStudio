...
governs:
  ...
  files:
    ...
    # The five single-tutorial frontend modules this ADR used to govern —
    # store/tutorialSlice.ts, components/TutorialPanel.tsx,
    # App.parts/useRunFirstWorkflowTutorial.ts, tutorials/runFirstWorkflow/content.ts,
    # and lib/api/tutorials.ts — were deleted by the Learning Center spec's
    # FR-001 and FR-002 rather than generalised, so the surface this ADR governs
    # is the system that replaced them.
...
happens by **running**, so a teaching entry is a real project (Section 2); a tool
library is **a place in the product**, not a filesystem convention (Section 3);
the way in starts from **what the user already has** (Section 4); and the
correctness of an imported block is **the user's to judge, with the product
saying so plainly** (Section 5).
...
percentage of the catalogue. `docs/specs/adr-053-learning-center.md` records the
revision, and #2056, #2057, and #2058 carry the work.
...
### 2.1 A Teaching Entry Is A Real Project

An entry that teaches does not simulate SciStudio and does not describe it. It
creates a project on disk, writes real input data into it, and walks the user
through building and running a workflow over that data. A review entry, which
teaches nothing new (Section 2.2), needs no project of its own.
...
here is that this is the standard for every entry that teaches, not a property of
...
for a package to teach its own blocks. #2056 tracks the shared entry-point
contract that discovery rests on, #2057 the system itself, and #2058 the first
core entry authored as manifest data.
...
those entries are real projects (Section 2.1), this is directly observable rather
than a proxy. Opening an entry and scrolling it is not progress, and the product
...
There is nothing to migrate from. The current implementation has never shipped in
a release users receive updates for, so it is removed outright with no
compatibility window, and the preferences it keeps — one entry's `completedAt`
and `dismissedAt` — live in the frontend store that backend-held progress
replaces. A migration would read a store no user running a released build has
ever written to, and would carry a completion of an entry that is itself
discarded, marking as done an entry the user has never seen.

This was weighed against #1986, which fixed a first-run landing that reappeared
on every launch because an ephemeral port reset the browser storage holding the
dismissal. That fix is preserved as behaviour: a user who dismisses the landing
is not shown it again. It is re-established in the backend store, where the
failure #1986 fixed cannot recur. The obligation #1986 establishes is that the
dismissal holds; the store it held in is incidental to that.
...
Its contents are entries completed by doing what they ask, which for an entry
that teaches means bootstrapping a project and running a workflow in it.
...
This work is tracked by #2001 and #2002 for the import path, and by #2057 for the
unlock that volunteers it.
...
- Implementation. This ADR is `Proposed` and `phase: planning`; all behavior it
  ...
  planned. Implementation is tracked by #1995 through #2002, and the Learning
  Center system by #2056, #2057, and #2058.
...
| Entries are on-disk data, not hardcoded identity | Adding a second entry requires a directory and a manifest, with no backend or frontend code change | #2057, #2058 |
| Listing the catalogue executes no package code | Discovery test over a package entry that declares a driver, failing if the module loads, and a shared entry-point contract that resolves the group without importing it | #2056, #2057 |
| A package entry cannot drive product behaviour | Progress test asserting the unlock and the toolbar indicator ignore non-core groups | #2057 |
| Completion requires doing what the entry asks | Progress test asserting that opening a teaching entry grants nothing, and that a review entry completes on being read | #2057 |
| Entry projects are unreachable except through the Learning Center | Test asserting absence from the project listing route while every other project route still resolves them | #2057 |
...
are specified by `docs/specs/adr-053-learning-center.md` and implemented under
#2056, #2057, and #2058.
This ADR
