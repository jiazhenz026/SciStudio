...
- Port data types are represented by port color, ring, hover title, and edge
  color. Type text is not duplicated inside the node body. (The standalone
  canvas TypeLegend component was removed in #1714 — superseded by the
  bottom-panel port info panel — so type semantics now live on the ports/edges
  and that panel rather than a separate legend overlay.)
...
> **NOTE 2026-08-07:** the three-button floating action toolbar shown in §2.2 and listed in §2.3 is **superseded** by §8 Addendum 1 below. The toolbar carries two buttons — run and delete. The restart button is withdrawn because it never did anything different from run.

## 8. Addendum 1 (2026-08-07): the node toolbar carries run and delete, not restart

### 8.1 Summary

The floating action toolbar drops its middle button. `[run] [restart] [delete]` becomes `[run] [delete]`.

| Item | §2.2 / §2.3 promise | Addendum 1 decision | Implementation issue |
|---|---|---|---|
| Node toolbar buttons | Three: run, restart, delete | **Two**: run, delete | #2033 |
| `onRestart` prop chain | Implied by the restart button, threaded `BlockNode` → `flowNodeBuilder` → `WorkflowCanvas` → `ProjectWorkspace` → `App` | **Removed** end to end, along with `handleRestartBlock` | #2033 |

### 8.2 Why

The two buttons were the same operation. `handleRunBlock` and `handleRestartBlock` in `App.parts/useWorkflowExecutionActions.ts` had identical bodies, down to the dependency array:
...
Both dispatched the same `executeFrom` call with the same arguments. What the user saw was two adjacent buttons with distinct icons and distinct tooltips — "Run block" and "Restart block" — implying a distinction the code did not make and could not explain. There is no state in which pressing one produces a different result from pressing the other.

The ADR listed three buttons because three seemed like the natural set for a node that can be running or finished. In practice `executeFrom` already covers both: it re-executes from that block regardless of prior state, which is what "restart" would have meant if it had been written separately. One button expresses that honestly.

This is the same correction as ADR-038 Addendum 1, one layer down: a second execution entry point that offered no second behavior. Both are handled by #2033.

### 8.3 Replaced text

##### §2.2 — the toolbar diagram
...
The rest of the diagram, the geometry rules, and the "actions float outside the square and do not change measured geometry" invariant are unchanged.

##### §2.3 — node-level interactions

The bullet "run, restart, delete, or open a node action menu" is **superseded** by:

> - run, delete, or open a node action menu;

### 8.4 Out of scope

- The Run control in the top toolbar, which runs the whole workflow, is a different operation and is untouched.
- "Run from here" semantics. The single remaining node button still calls `executeFrom`, which is exactly what it called before; only the duplicate entry point is gone.
- The unified status surface (§2.5) and its click-to-details behavior.
