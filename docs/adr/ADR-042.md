...
class Phase(StrEnum):
    """Historical phase enum retained for audit record compatibility.

    ADR-042 section 26 is reserved for an owner-authored replacement plan.
    """
    PLANNING = "planning"
    PHASE_0 = "phase-0"
    PHASE_1 = "phase-1"
...
## 26. Implementation Plan (owner rewrite pending)

The ADR-042/043/044 rollout plan was intentionally cleared on 2026-05-18
after the project owner decided to re-plan the implementation manually.

This section is now a reserved implementation-plan slot. It does not
authorize any phase, sub-track, CI flip, cleanup sprint, or agent dispatch
until the owner lands a replacement plan through the normal review path.

Until that replacement lands:

- ADR-042/043/044 remain accepted design/governance documents.
- No Phase 1, Phase 1.5, Phase 2, Phase 3, Phase 4, or Phase 5 completion
  claim may be inferred from the removed plan.
- Historical implementation PRs after #1120 remain in git history for
  reference/cherry-pick, but are not treated as the active plan of record.
