---
adr: 31
title: "Data Object Reference-Only Contract, ViewProxy Elimination, And Lazy Loading Enforcement"
status: Accepted
date_created: 2026-04-11
date_accepted: 2026-04-11
date_superseded: null
supersedes: [27]
superseded_by: null
related: [7, 17, 27, 28, 32, 38]
closes_issues: []
tracking_issue: 1289
...
assisted_by:
  - "Codex:gpt-5"
phase: legacy
tags: [dataobject, storage, lazy-loading, viewproxy, io]
owner: "@jiazhenz026"
co_authors: ["@codex"]
language_source: en
translations: []
...
ADR-031 establishes `DataObject` instances as reference-only transport objects,
eliminates ViewProxy as a separate input/accessor layer, and requires loaders
and derived array operations to preserve lazy storage-backed behavior.

### 1.1 Problems Addressed

| Problem | Risk | ADR-031 response | Detailed section |
|---|---|---|---|
| Data payloads could cross block boundaries through in-memory attributes | Subprocess transport becomes eager and memory unsafe | Require reference-only `DataObject` transport | Section 2 |
| ViewProxy duplicated access semantics | Runtime and docs split lazy loading across two concepts | Absorb access methods into `DataObject` and `Array` | Section 2 |
| Loaders and savers could bypass storage references | Save/load round trips lose laziness and metadata | Require IO blocks to persist and return storage-backed objects | Section 2 |
...
### Addendum 3: ADR-038 supersession bridge (2026-05-15)

ADR-038 (unified run lineage database, supersedes ADR-032) introduces a `lineage.db.data_objects` table with a `wire_payload TEXT NOT NULL` column whose content is **the same reference-only wire format defined by this ADR**. Specifically:

- The reference-only `DataObject` envelope this ADR mandates (`framework` / `meta` / `user` / `storage_ref` slots, no in-band payload) is what `_serialise_one()` produces and what gets stored verbatim in `lineage.db.data_objects.wire_payload` per ADR-038 §3.1.
- A 1000-item `Collection` produces 1000 rows in `data_objects` and `block_io`; each `wire_payload` is KB-scale (per this ADR's Phase 1 measurement of ≈1-2 KB per envelope) per ADR-038 §3.1 prose, so the total storage cost is ~MB, not GB.
- The contract is **unchanged** by ADR-038. ADR-031 defines what the wire format IS; ADR-038 just persists it durably and queryably. No code on the block-author side changes.

Cross-reference: ADR-038 §3.1 (schema) + §3.5 (no-per-run-isolation rationale + KB-scale storage argument).
