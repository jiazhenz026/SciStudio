...
planned_governs:
  ...
  files:
    ...
    # ``tests/packages/**`` was planned here and now resolves: ADR-053's
    # Learning Center work created the directory for the entry-point symmetry
    # test its spec §4.4 names. The directory existing says nothing about this
    # ADR's own planned surfaces, which remain unbuilt, so the glob moves out
    # rather than the whole block advancing. This ADR's package-validator tests
    # will land beside it under the same path.
  excludes: []
