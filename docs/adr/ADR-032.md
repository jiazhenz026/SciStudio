---
adr: 32
title: "Project-Level Metadata Store - SQLite Persistent Mirror Of DataObject Metadata"
status: Superseded
date_created: 2026-04-11
date_accepted: null
date_superseded: 2026-05-15
supersedes: []
superseded_by: 38
related: [7, 17, 27, 31, 38]
closes_issues: []
tracking_issue: 1289
...
governs:
  modules:
    ...
    - scistudio.core.lineage.store
  contracts:
    - scistudio.core.metadata_store.MetadataStore
    - scistudio.core.metadata_store.get_metadata_store
    - scistudio.core.metadata_store.set_metadata_store
...
assisted_by:
  - "Codex:gpt-5"
phase: legacy
tags: [metadata, sqlite, lineage, superseded]
owner: "@jiazhenz026"
co_authors: ["@codex"]
language_source: en
translations: []
...
ADR-032 defined a project-level SQLite metadata mirror for `DataObject`
identity and metadata. It is superseded by ADR-038, which collapses this store
into the unified `.scistudio/lineage.db` model.

### 1.1 Problems Addressed

| Problem | Risk | ADR-032 response | Detailed section |
|---|---|---|---|
| DataObject metadata did not survive project reopen or restore | Reproducibility and previews lost object identity | Introduce a project-level metadata store | Section 2 |
| Separate metadata and lineage stores later overlapped | Users and code had two history databases | ADR-038 supersedes this ADR with unified lineage storage | Section 2 |

## 2. Existing ADR Body
## ADR-032: Project-Level Metadata Store — SQLite Persistent Mirror of DataObject Metadata

> **⚠️ SUPERSEDED by ADR-038 (2026-05-15).** The `<project>/metadata.db` store described
> below has been collapsed into a single `<project>/.scistudio/lineage.db` with four
> normalized tables (`runs`, `block_executions`, `data_objects`, `block_io`). The
> DataObject identity catalog (this ADR's core deliverable) now lives in the
> `data_objects` table of the unified store. The terminology "metadata.db" was a
> source of user-facing confusion — see ADR-038 §2.2 for the rationale.
>
> This document is preserved unmodified for historical context. Any new work should
> consult ADR-038 instead. The `src/scistudio/core/metadata_store.py` module is
> replaced by a 6-month deprecation shim that re-exports the unified store with a
> `DeprecationWarning`.

**Status**: **superseded by ADR-038**
**Date**: 2026-04-11
**Superseded**: 2026-05-15
**Related**: ADR-007 (lazy loading), ADR-017 (cross-process transport), ADR-027 D5 (three-slot metadata), ADR-031 (reference-only contract), **ADR-038 (supersedes this)**
...
User runs workflow → intermediate data in `data/zarr/` → closes SciStudio → reopens → clicks "continue from block X" → checkpoint loads StorageReference → backend reads zarr → DataObject reconstructed with shape/dtype/axes only → `framework.object_id`, `meta.pixel_size`, `user.experiment_id` all gone → downstream block calling `item.meta.pixel_size` crashes.
...
| File | Purpose |
|------|---------|
| `src/scistudio/core/metadata_store.py` | MetadataStore class + `get_metadata_store()` singleton accessor |

#### 4.2 Modified files

| File | Changes |
|------|---------|
| `src/scistudio/blocks/base/block.py` | `_auto_flush()`: after `obj.save()`, call `store.put(obj)` (D3) |
| `src/scistudio/blocks/io/io_block.py` | `run()`: after D4 safety net flush, call `store.put(item)` for loader outputs |
| `src/scistudio/engine/checkpoint.py` | Restore path: use `store.get(object_id)` for full metadata reconstruction (D4) |
| `src/scistudio/engine/scheduler.py` | Pass `workflow_id`/`block_id` context for bookkeeping columns |
| `src/scistudio/engine/runners/worker.py` | `serialise_outputs()`: after auto_flush + serialise, call `store.put()` if store available |
| `src/scistudio/api/runtime.py` | Initialize MetadataStore with project path when project opens |
