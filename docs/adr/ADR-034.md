...
- Rename a tab by double-clicking its title, or by clicking the pencil that
  appears on the focused tab while hovered (#1994 — double-click alone was
  undiscoverable)
...
## 4. Amendment 2: Common Project Instruction Entry (2026-09-14)

### 4.1 Owner Direction

The owner directed SciStudio to remove the Claude-only prompt-injection path and
place the common embedded-agent knowledge in the provisioned project
`AGENTS.md`. This amendment is the focused authorization to update this
`agent_editable: false` ADR for issue #2383.

### 4.2 Decision

Every embedded provider starts in the provisioned project root and uses
`AGENTS.md` as the common identity, rules, and navigation entry. `CLAUDE.md` and
the packaged base `scistudio` skill are short routers to that file for clients
that discover different filenames or skill trees. Detailed task procedures stay
in task skills and version-matched references.

The provider registry no longer models system-prompt injection, and
`spawn_agent` no longer creates a prompt tempfile or passes
`--append-system-prompt`. MCP injection and positional initial-task delivery
remain descriptor-driven and unchanged.

Runtime project state and tool contracts come from the live MCP server. The
common guide tells agents to query current facts and inspect MCP tool schemas;
it does not duplicate a fixed tool catalogue or freeze a startup snapshot.

### 4.3 Governed-Surface Correction
