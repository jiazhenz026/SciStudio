...
supersedes: []
superseded_by: null
related: [17, 19, 22, 34, 36, 40, 41, 48, 51, 52, 53, 54]
closes_issues: []
tracking_issue: 2239
...
phase: planning
tags: [webmcp, external-ai-host, deployment, multi-user, desktop, enterprise-seam]
owner: "@jiazhenz026"
co_authors: []
language_source: en
translations: []
...
Lab server. Local users choose desktop use or external AI use at startup. The
open-source edition serves the first and keeps its present behavior:
`scistudio serve` binds every interface and asks for no login. Multi-user Lab
deployment is provided by a separate enterprise edition, `scistudio-enterprise`.
Its users log in through JupyterHub, which routes them to their own SciStudio
backend managed through systemd; each user has one backend, one bundled Python,
and one runtime environment shared by that user's projects. The enterprise
edition has its own launch command, builds the standard backend through the
open-source app factory, and adds its own guard, routes, tools, and background
tasks. This repository owns the seam that edition builds on, not the deployment
itself (Section 8). Docker and per-project dependency isolation are not
...
The AI-host frontend uses the owner-selected layout recorded in Section 6.
The demo is a visual reference only; its temporary implementation, badges,
and tutorial adaptations are excluded. Browser
...
This draft does not mark the implementation issue complete. Section 8 was
amended on September 11, 2026, when the owner moved multi-user Lab deployment
to the enterprise edition and kept the open-source edition as it is
([issue #2304](https://github.com/jiazhenz026/SciStudio/issues/2304)).
...
| Lab data stays on Linux servers while scientists work through their own computers | Desktop and personal CLI-agent setup obstruct access | Keep the open-source backend composable through an identity seam; the enterprise edition provides Hub identity, per-user native backend instances, and browser access on it | Section 8 |
...
The deployments in scope are local machines, served by the open-source edition,
and internal Lab servers, served by the enterprise edition built on it. Public
...
| API and MCP context track the active project within a backend | `api/runtime/_projects.py`, `ai/agent/mcp/_context.py` | Protect project selection during calls; the enterprise edition binds authenticated users to separate backends |
...
The stdio adapter `scistudio webmcp-adapter` (issue #2308) is the bridge's
second consumer. AI apps that run local MCP servers but expose no WebMCP, such
as Claude Desktop, Claude Code and Codex, launch it over stdio, and it forwards
`tools/list` and `tools/call` to the same two routes. It sees the same
catalogue, the external-audience tools included, and returns results through
the same adapter contract and project binding. It adds no second registry and
no new server transport. For CLI agents it is the path to the external-audience
tools; the local MCP path above remains for everything else. How it
authenticates is specified in `adr-055-enterprise-support`: a per-user
loopback token file for a backend on the same computer, or a bearer credential
that an edition's guard validates.
...
SciStudio provides two presentations over shared domain components, contracts,
and backend state synchronization. Electron and ordinary browsers use the full
workbench. The AI-host presentation keeps the canvas/editor stage, moves the
library sidebar and activity rail to the right, adds a distinct **Preview**
section alongside **Previewers**, and removes the AI Chat entry. Other bottom
panel tools remain available. Selecting a node or producing a plot reveals the
Preview section in AI mode.

The September 11, 2026 owner decision adopts only this interaction/layout idea
from `scistudio-web-demo`. The production implementation uses the current core
components; it does not transplant temporary demo behavior or repurpose the
Previewers catalogue as Preview.

The page URL explicitly selects presentation: `?ui=ai` selects the AI layout;
`?ui=workbench`, missing values, and unknown values select the workbench. The
external-AI launcher includes `ui=ai` in its copyable address. In browsers, a toolbar switch
updates the current page's URL without reloading the project, preserving other
query parameters, service prefixes, and fragments. Presentation is page-local,
not a backend mode or a shared browser-storage preference.

Host identity, WebMCP availability, deployment mode, and viewport size remain
separate inputs. A WebMCP capability does not switch layout automatically.
The same backend can serve pages using different layouts concurrently.
Already-mounted terminal sessions survive in-place presentation changes;
direct AI entry does not initialize the hidden AI Chat surface.

The implementation and acceptance cases are specified in
[AI-host presentation](../specs/adr-055-ai-host-presentation.md).
...
## 8. Internal Lab Deployment Through The Enterprise Edition

Multi-user Lab deployment is not part of the open-source edition. On September
11, 2026 the owner decided that the open-source edition keeps its present
behavior, with `scistudio serve` binding every interface and asking for no
login, and that a separate private package, `scistudio-enterprise`, provides
the Lab deployment. This section describes that deployment, because its needs
shape the open-source backend, and then what the open-source edition provides
for it.

### 8.1 The Deployment The Enterprise Edition Provides

The Lab deployment consists of JupyterHub for identity and routing,
...
configuring personal CLI agents on the server. The account-management
experience reuses JupyterHub and an existing authenticator; service users are
provisioned or mapped by the deployment layer and keep ownership of their
workspaces, without needing Unix accounts or SSH access. Authentication covers
the UI, APIs, WebSocket connections, the WebMCP bridge, and transfer
endpoints, including protection against bypass through a raw backend port.
...
state, and all of that user's projects share their environment, so upgrading a
shared dependency can affect their other projects by design. Automatic
per-project environments, dependency locking, and backend/environment
switching on project selection are outside scope; explicit CodeBlock
interpreter configuration remains available. Workspaces and results persist
independently of browser and service lifetimes. Shared datasets use configured
directories and filesystem permissions. Systemd manages service logs and
process groups and applies administrator-configured resource limits, which must
be verified on the supported deployment rather than assumed from configuration.
The [SystemdSpawner requirements](https://github.com/jupyterhub/systemdspawner#requirements)
include a Linux systemd host and root execution for the Hub component; the user
backends run under ordinary accounts. Browser disconnection and idle-page
detection do not terminate active analyses or transfers. Docker images and a
container deployment product are outside this deployment.

These are the enterprise edition's decisions to carry out, and its
specifications live with it. The Lab deployment spec first written under this
ADR leaves this repository with the deployment it describes, and the changes
the open-source repository makes for that edition are collected in
`adr-055-enterprise-support`
([issue #2303](https://github.com/jiazhenz026/SciStudio/issues/2303)).

### 8.2 The Seam In The Open-Source Edition

The enterprise edition has its own launch command, which the Hub spawner runs.
That command builds the standard backend through the open-source factory,
`scistudio.api.app.create_app`, and composes its additions onto it. Nothing is
discovered or loaded automatically; an earlier design had the open-source
`serve` load identity backends from an entry-point group, and the owner
replaced it with this composition (issue #2304, option B). The factory accepts:

- a replacement guard in place of the WebMCP bridge's loopback token
  middleware; the guard decides which paths it protects, and the Hub guard
  protects everything, the WebSocket included;
- lifespan hooks for startup checks, such as validating the service prefix and
  OAuth callback at spawn, and for background tasks, such as reporting activity
  to the Hub while runs or transfers are active;
- a capability declaration, `identity` and `transfer`, that the frontend reads
  at boot. The enterprise edition's UI pieces live in the open-source frontend,
  hidden until the matching capability is declared;
- the edition's own routers.

Open-source modules can also register path prefixes whose routes authenticate
requests themselves, and every guard lets those requests through to the route.
ADR-054's per-mount panel tokens are the first such prefix; the
token-authenticated exception that ADR-054 planned to record in the Lab spec is
recorded in the seam spec instead.

With nothing passed, the factory builds the open-source backend exactly as
before. The enterprise edition depends on open-source internals, the factory
and the shared MCP registry, so a change there can need a matching change in
that edition. The owner accepted this trade-off. To make such changes visible
rather than silent, everything the enterprise edition relies on is declared
`provisional` public API under ADR-052. A reusable contract suite, run here
against a test-only fake guard, checks any replacement guard at the root mount
and under a service prefix, and the enterprise guard runs the same suite. The
implementation spec is `adr-055-identity-seam`.
...
startup, and the identity seam the enterprise edition's Lab deployment is built
on. The Lab deployment itself, with Hub identity, per-user instances, and
deployment assets, belongs to the enterprise edition (Section 8). The
implementation specs and resulting tasks remain tracked by issue #2239 and this
ADR.
...
| ADR-052: public API contract | Declare the surface the enterprise edition composes on as provisional public API, in the canonical roots `scistudio.api.app` and `scistudio.api.seam` |
| ADR-054: panels | Register the per-mount panel token paths as self-authenticating, so panel files load under any guard (#2288) |
...
explicit. The architecture overview is not edited by this ADR. The generated
API reference is regenerated, never hand-edited, when the declared surface
changes.
...
automatic installation of host-native hooks, adoption of demo-specific runtime
and tutorial behavior beyond the Section 6 layout decision, multi-user login in
the open-source edition, and automatic loading of identity backends from entry
points.
...
| Seam default | `create_app()` with no arguments builds the open-source backend as before, at the root mount and under a service prefix |
| Seam contract | A replacement guard covers the shell, APIs, WebSocket, and bridge; self-authenticating prefixes reach their routes; lifespan hooks and capabilities behave as specified; the test-only fake guard passes the contract suite here and the enterprise guard passes it in its own repository |
...
Lab identity and lifetime evidence, including two users on separate backends,
raw-port and WebSocket bypass attempts, restart and browser-close behavior, and
idle management that observes running jobs and transfers, belongs to the
enterprise edition, which runs the deployment. The implementation specs under
this ADR are `adr-055-prefix-independence` (Spec 0), `adr-055-webmcp-bridge`
(Spec 1), `adr-055-agent-context-workspace` (Spec 2),
`adr-055-local-background-runtime` (Spec 3), and `adr-055-identity-seam`, the
seam in Section 8.2.

Host verification here covers loopback access; the internal Lab's
secure-context and authentication configuration is verified by the enterprise
edition. The
...
retaining backend ownership of its workflows and data operations. The
enterprise edition's Lab deployment adds identity and native service management
without introducing container image operations, and it does so on a seam rather
than a fork: the open-source backend stays single-user, and the enterprise
edition depends on a declared, provisional surface of it. A change to that
surface can require a matching enterprise change; the provisional marking puts
each one in the changelog instead of letting it break the enterprise edition
silently. Administrators still own system-level setup, user permissions,
...
| Load identity backends into the open-source `serve` from an entry-point group | Replaced on September 11, 2026 (option B): the enterprise edition has its own launch command and composes onto `create_app`, and nothing loads automatically |
| Add multi-user login to the open-source edition | Excluded; the open-source edition keeps its single-user behavior, and the enterprise edition provides the Lab deployment |
