...
| `adr_implementation_check` CI/pre-commit wiring | [x] | CI job `ADR Implementation Tracker` runs `python scripts/audit/adr_implementation_check.py --json` with `PYTHONPATH=src`; warnings remain non-blocking, errors fail | Future owner pass may tighten warning policy or add PR-aware diff enforcement | Wired in `.github/workflows/ci.yml`; no editable install |
