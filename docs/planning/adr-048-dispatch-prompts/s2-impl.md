...
- CodeBlock runner reuse (IMPORT ONLY): `scistudio.blocks.code._backends_registry.run_codeblock_process(argv, cwd, env_delta, timeout_seconds)` (raises `CodeBlockTimeoutError`); interpreter resolution in `interpreters.py`; R via `runners/r_runner.py` / `backends/r_quarto.py`.
