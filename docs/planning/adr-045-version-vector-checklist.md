...
| A3 | implementer | N/A | `docs/planning/dispatch-prompts/adr-045-a3-frontend-reconcile.md` | Frontend workflow/file reconcile, source_id handling, conflict state | `feat/issue-1401/adr-045-frontend-reconcile` | `.claude/worktrees/adr-045-a3-frontend-reconcile` | frontend API/websocket/store/tests assigned in prompt; manager-added CI/gate fixes in API contract constants and frontend mocks | backend persistence and event emission except manager-owned CI/gate fixes | #1401; PR #1443 | `[x]` PR https://github.com/zjzcpj/SciStudio/pull/1443 merged to tracking at `f5cbf904`; gate `.workflow/records/1401-a3-frontend-reconcile.json`; manager-reviewed save-start `source_id`, websocket-echo dirty clearing, clean remote-delete conflict edges, `projects.py` BOM removal, `workflow_watcher`/`runtime` Sentrux cycle break via `file_contracts`, first-party file watcher echo suppression, unified `tests/contract` ADR-045 coverage, and CodeQL sandbox-precondition annotations; frontend full suite 46 files / 481 tests passed; frontend build passed; architecture tests 328 passed; backend race/API suite 22 passed; unified contract+race xdist regression 14 passed; Sentrux free-tier 3/15 rules passed; full audit `docs/audit/adr-045-a3-full-audit.json` passed; PR #1443 CI all passed. |
...
- [x] A3 frontend reconciliation output reviewed. -> PR #1443 merged to tracking; Codex P1/P2 accepted and fixed; backend `file.changed` contract gap, frontend save/conflict edge regressions, unified contract tests, and CI/CodeQL blockers addressed.
...
| 2026-05-22 | A3 / Godel + manager | local | `[~]` | Resumed on tracking `46075f91`; frontend API/store/WebSocket reconcile and deterministic tests cover stale drop, source_id self-echo, save-start source tracking, websocket-echo dirty clearing, clean remote adoption, dirty conflict preservation, clean/dirty remote delete conflicts, gitRestore refresh, and file.changed handling. Targeted Vitest suite: 4 files / 30 tests passed; build passed; A3 did not add backend integration files because manager landed them on tracking. |
| 2026-05-22 | manager | local | `[~]` | Fixed latest PR #1443 CI/gate blockers: removed UTF-8 BOM from `projects.py`, updated two frontend API mocks for `setWorkflowWriteStartedListener`, moved `FILE_ENTITY_CLASS` into `file_contracts` to break the `workflow_watcher` -> `runtime` import edge, recorded docs/changelog N/A and Sentrux pass evidence. Local checks: frontend full suite 46 files / 481 tests passed; build passed; architecture 328 passed; backend race/API 22 passed; Sentrux 3/15 free-tier rules passed. |
| 2026-05-22 | manager | local | `[~]` | Fixed PR #1443 Python CI duplicate `file.changed` race by recording a pending first-party file signature before atomic replace and adding `test_first_party_file_write_pending_signature_suppresses_watcher_echo`. Local xdist regression: `pytest -n auto tests/api/test_file_version_vector.py tests/api/test_reload_on_save.py tests/integration/test_race_external_editor.py --timeout=60 --timeout-method=thread --no-cov` -> 11 passed. |
| 2026-05-22 | manager | local | `[~]` | Added unified ADR-045 contract entry point at `tests/contract/test_adr045_version_vector_contract.py` for HTTP response `state_version`, event payload `version`, workflow schema `version`, `source_id`, WebSocket forwarding, and first-party watcher echo suppression. Local xdist regression with contract suite: 14 passed. |
| 2026-05-22 | manager | local | `[~]` | Addressed umbrella PR #1410 CodeQL path-injection alerts with inline sandbox-precondition annotations at ADR-045 first-party path signature operations; no CI workflow, Sentrux, or security threshold changes. Local checks: ruff/format pass and unified contract+race xdist regression 14 passed. |
...
| 2026-05-22 | #1443 | `chatgpt-codex-connector` | P1 | `markFileRemoteConflict` marked clean remote delete/move conflicts dirty, which could trigger file autosave and recreate stale deleted content. | Accept. Preserve clean tabs as clean for remote delete/move conflicts; keep dirty tabs dirty; add store and WebSocket regression tests. | manager | `[x]` |
| 2026-05-22 | #1443 | `chatgpt-codex-connector` | P2 | `markWorkflowSaved` left `workflowDirty` true after successful save while waiting for the WebSocket echo, causing repeated autosave writes if the echo was delayed/disconnected. | Accept. Track save-start `source_id`, clear dirty for the saved local version, and keep dirty only when a newer local edit occurs during the in-flight save; add regression tests. | manager | `[x]` |
