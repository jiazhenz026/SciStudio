...
- In scope:
  - Split `docs/adr/ADR.md` so ADR-001 through ADR-030 each has a standalone
    `docs/adr/ADR-<NNN>.md` file.
  ...
  - Make code and function signatures auditable when an ADR has normative code
    contracts.
  - Replace or reduce `docs/adr/ADR.md` only after every extracted ADR is
    owner-reviewed and link coverage is checked.
- Out of scope:
...
- [x] Source mega-document identified. -> `docs/adr/ADR.md`
...
- In scope:
  - Extract each ADR from `docs/adr/ADR.md`.
  ...
  - Preserve source content while improving structure, traceability, and audit
    contract placement.
- Out of scope:
  - Bulk deletion of `docs/adr/ADR.md` before all extracted files are reviewed.
- Required docs:
...
### 8.3 Track C - ADR.md Decomposition Closure

- Owner: `manager`
- In scope:
  - Verify every ADR-001 through ADR-030 section has a standalone successor.
  - Replace the mega-document with an index, historical pointer, or remove it
...
- Required docs:
  - `docs/adr/ADR.md` final disposition.
- Required checks:
  - Link/closure audit.
  - Full audit.
...
|---|---|---|---|---|---|
| ADR-001: Six base data types with inheritance | `docs/adr/ADR.md`; draft exists | `docs/adr/ADR-001.md` | `[x]` | `[x]` | Owner approved; frontmatter lint pass; ADR-001 signature drift smoke pass |
| ADR-002: Named axes on Array types | `docs/adr/ADR.md` | `docs/adr/ADR-002.md` | `[x]` | `[x]` | Owner approved; captures ADR-027 axes refinement; frontmatter lint pass; ADR-002 signature drift smoke pass |
| ADR-003: Broadcast as explicit utility, not implicit type-system behaviour | `docs/adr/ADR.md` | `docs/adr/ADR-003.md` | `[x]` | `[x]` | Owner approved; frontmatter lint pass; ADR-003 signature drift smoke pass |
| ADR-004: Five block categories plus SubWorkflowBlock | `docs/adr/ADR.md` | `docs/adr/ADR-004.md` | `[x]` | `[x]` | Owner approved; batch frontmatter lint pass; batch signature drift smoke pass |
| ADR-005: CodeBlock supports inline and script execution modes | `docs/adr/ADR.md` | `docs/adr/ADR-005.md` | `[x]` | `[x]` | Owner approved; notes ADR-041 CodeBlock v2 narrowing; batch frontmatter lint pass; batch signature drift smoke pass |
| ADR-006: External software integration via file-exchange bridge | `docs/adr/ADR.md` | `docs/adr/ADR-006.md` | `[x]` | `[x]` | Owner approved; batch frontmatter lint pass; batch signature drift smoke pass |
| ADR-007: Lazy loading by default via ViewProxy | `docs/adr/ADR.md` | `docs/adr/ADR-007.md` | `[x]` | `[x]` | Owner approved; notes ADR-031 ViewProxy elimination; batch frontmatter lint pass; batch signature drift smoke pass |
| ADR-008: Two-tier block and type distribution | `docs/adr/ADR.md` | `docs/adr/ADR-008.md` | `[x]` | `[x]` | Owner approved; batch frontmatter lint pass; batch signature drift smoke pass |
| ADR-009: Registry stores specs, not class references | `docs/adr/ADR.md` | `docs/adr/ADR-009.md` | `[x]` | `[x]` | Owner approved; batch frontmatter lint pass; batch signature drift smoke pass |
| ADR-010: Batch execution mode declared per block | `docs/adr/ADR.md` | `docs/adr/ADR-010.md` | `[x]` | `[x]` | Owner approved; rewritten as Superseded by ADR-020; batch frontmatter lint pass; inactive ADR skipped by signature extraction |
| ADR-011: Workflow definition as declarative YAML, decoupled from frontend | `docs/adr/ADR.md` | `docs/adr/ADR-011.md` | `[x]` | `[x]` | Owner approved; batch frontmatter lint pass; batch signature drift smoke pass |
| ADR-012: Checkpoint-based pause and resume | `docs/adr/ADR.md` | `docs/adr/ADR-012.md` | `[x]` | `[x]` | Owner approved; batch frontmatter lint pass; batch signature drift smoke pass |
| ADR-013: AI as a four-tier service layer, not embedded in the core | `docs/adr/ADR.md` | `docs/adr/ADR-013.md` | `[x]` | `[x]` | Owner approved; batch frontmatter lint pass; batch signature drift smoke pass |
| ADR-014: ReactFlow + FastAPI as the frontend-backend stack | `docs/adr/ADR.md` | `docs/adr/ADR-014.md` | `[x]` | `[x]` | Owner approved; notes ADR-039 project state update; batch frontmatter lint pass; batch signature drift smoke pass |
| ADR-015: Inclusive strategy - wrap existing tools, never replace | `docs/adr/ADR.md` | `docs/adr/ADR-015.md` | `[x]` | `[x]` | Owner approved; batch frontmatter lint pass; batch signature drift smoke pass |
| ADR-016: Per-port InputDelivery for CodeBlock data handoff | `docs/adr/ADR.md` | `docs/adr/ADR-016.md` | `[x]` | `[x]` | Owner approved; rewritten as Superseded by ADR-020; batch frontmatter lint pass; inactive ADR skipped by signature extraction |
| ADR-017: Subprocess isolation for all block execution | `docs/adr/ADR.md` | `docs/adr/ADR-017.md` | `[ ]` | `[ ]` | Check worker/process contract signatures |
| ADR-018: Block cancellation, graceful workflow degradation, and event-driven runtime | `docs/adr/ADR.md` | `docs/adr/ADR-018.md` | `[ ]` | `[ ]` | Check event/cancellation contracts |
| ADR-019: ProcessHandle, ProcessRegistry, and cross-platform process lifecycle | `docs/adr/ADR.md` | `docs/adr/ADR-019.md` | `[ ]` | `[ ]` | Likely needs process lifecycle symbols |
| ADR-020: Collection-based data transport - eliminate engine-level batch iteration | `docs/adr/ADR.md` | `docs/adr/ADR-020.md` | `[ ]` | `[ ]` | Check collection transport contracts |
| ADR-021: MergeCollection and Collection operation blocks | `docs/adr/ADR.md` | `docs/adr/ADR-021.md` | `[ ]` | `[ ]` | Check block API contracts |
| ADR-022: OS-level memory monitoring replaces estimated memory budget | `docs/adr/ADR.md` | `docs/adr/ADR-022.md` | `[ ]` | `[ ]` | Check monitoring boundary and platform notes |
| ADR-023: Frontend layout redesign | `docs/adr/ADR.md` | `docs/adr/ADR-023.md` | `[ ]` | `[ ]` | Docs-only unless UI contract is normative |
| ADR-024: Frontend bundling, SPA serving, and `scieasy gui` command | `docs/adr/ADR.md` | `docs/adr/ADR-024.md` | `[ ]` | `[ ]` | Likely needs CLI command contract |
| ADR-025: Block package distribution protocol with entry-points | `docs/adr/ADR.md` | `docs/adr/ADR-025.md` | `[ ]` | `[ ]` | Check entry-point contract symbols |
| ADR-026: Block SDK - scaffolding, test harness, and developer documentation | `docs/adr/ADR.md` | `docs/adr/ADR-026.md` | `[ ]` | `[ ]` | Check SDK CLI/API signatures |
| ADR-027: Phase 10 core type system and block runtime refinements | `docs/adr/ADR.md` | `docs/adr/ADR-027.md` | `[ ]` | `[ ]` | Preserve addendum/supersession links |
| ADR-028: IOBlock architectural refactor - plugin-owned IO pattern | `docs/adr/ADR.md` | `docs/adr/ADR-028.md` | `[ ]` | `[ ]` | Check IOBlock contracts |
| ADR-029: Variadic port count and per-instance port editor | `docs/adr/ADR.md` | `docs/adr/ADR-029.md` | `[ ]` | `[ ]` | Check port editor/runtime split |
| ADR-030: config_schema MRO merge and base-class field injection | `docs/adr/ADR.md` | `docs/adr/ADR-030.md` | `[ ]` | `[ ]` | Likely needs config schema signature contracts |
...
- [ ] `docs/adr/ADR.md` no longer acts as the governing mega-document.
