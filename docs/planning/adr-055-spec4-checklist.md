...
|---|---|---|---|---|---|---|---|---|---|---|
| `A1` | `implementer` | `N/A` | prompts §A1 | O1: capability extensions and enterprise UI, plus #2328 and the `ai_pty` internal prefix | `feat/2322-enterprise-ui` | `.worktrees/feat-2322-enterprise-ui` | see prompts §A1 | `webmcp.py`, `cli/**`, enterprise code | `#2322`, `#2328`, PR #2336 | `[~]` Fix round head 07c474836. It covers the P1 fix (strictly-below prefix matching plus the reserved tab id), the restart race and `confirm_active_runs`, upload staging semantics, URL and path hardening, the shared resolver, and the ratchet fix (commits 5ad2720e8, 7e116e7df, c063860ea). Both audit branches are merged. AU1 passed its re-verification and AU2 passed with fixes. Final round, head dd51834ba: re-verification reports merged (93c3a05ab), plus the CHANGELOG refused-areas line (9f2273842). Main merge (#2343 docstring guard) at head 0e8546ff3; the PR is mergeable. The conflicts were confined to docstrings. Nine branch docstrings were rewritten, and the `docstrings` check shows 0 findings. The reference pages were regenerated with main's generator. CI is 17/17 green at 0e8546ff3 and the PR is mergeable (CLEAN). Re-synced with main after #2329 merged (133d00313, head 5ae1a917b); the only conflict was the spec `planned_governs` list. CI is 17/17 green at 5ae1a917b and the PR is mergeable (CLEAN). After it, the PR is ready for owner merge, pending owner approval of the #2337 deferrals. |
| `A2` | `implementer` | `N/A` | prompts §A2 | O2: stdio MCP adapter and loopback token file, plus #2333 | `feat/2308-webmcp-adapter` | `.worktrees/feat-2308-webmcp-adapter` | see prompts §A2 | `seam.py`, `spa.py`, `app.py`, `ai_pty/**`, `frontend/**` | `#2308`, `#2333`, PR #2329 | `[~]` Fix round head 26af14cf8. CI is 17/17 green and the PR is mergeable. Changes in the round: P1 fix (a per-request project snapshot), token-file loopback-only with proxies off, live-file protection, the ADR-055 §4 paragraph, the bridge pointer ownership check, all P3s, a main merge (40a0718cf), and CodeQL SHA-256 (24b20e4bb). Commits: 2dcf3b650 and 9cd55a627. The Windows token-file ACL gap is added to #2342. AU3b re-verification at 26af14cf8 **passes** (7e2575793). Live verification that hosts re-list tools is filed as a follow-up. AU4b re-audit at 26af14cf8 is **pass-with-fixes** (a90d09f50). Its remaining P2, the `_publish_mcp_port` pointer write, is being fixed in #2334 by A3. The Windows items are tracked in #2342. A2's final round is done at head f4e3199cf (changes in ee09dc444):
- both re-verification reports merged (bbaffc15d);
- the re-list wording;
- the unknown-outcome restart, catalogue restart and real-subprocess stdio tests;
...
| 2026-09-11 | manager | #2334 at b496cbf94 fails `Test (Python 3.11/3.13)` on 2 tests in `tests/engine/test_run_logging.py`: the run log lands in user-data `logs` instead of `<project>/.scistudio/logs`. Main at aedb83c19 is green, so the regression is in the #2351/#2352 round (504b2605c, cf00fc514). | Routed to A3. Root cause: the new test `tests/cli/test_graceful_shutdown_bound.py` ran `gui --bundled` in-process, which leaves `SCISTUDIO_BUNDLED=1` set, and it did not restore the env. That leaked into `tests/engine` in the same worker. The product code is not at fault. Fix: a test-only autouse restore fixture (0770595b4), head 98b99acf7, with the run-logging tests unchanged. CI is 17/17 green at 98b99acf7, including the first Linux run of the POSIX SIGTERM stop test, and the PR is mergeable (CLEAN). | #2352 |
