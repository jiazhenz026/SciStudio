...
| 2026-09-11 | manager | #2334 at b496cbf94 fails `Test (Python 3.11/3.13)` on 2 tests in `tests/engine/test_run_logging.py`: the run log lands in user-data `logs` instead of `<project>/.scistudio/logs`. Main at aedb83c19 is green, so the regression is in the #2351/#2352 round (504b2605c, cf00fc514). | Routed to A3 with the diff range de11b2848..b496cbf94. Fix the root cause without weakening the tests. A new project without `.scistudio/` must still log under it. | #2352 |
