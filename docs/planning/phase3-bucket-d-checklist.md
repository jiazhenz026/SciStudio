...
| `D3'` | `implementer` | `N/A` | inline | Class-binding split of `core/versioning/git_engine.py` 849 LOC + write `docs/adr/ADR-046-addendum1.md` in same PR | `refactor/issue-1472/git-engine-helpers` | `.claude/worktrees/phase3-d3-git-engine` | new `src/scistudio/core/versioning/_{commit,history,branch,status,merge}_ops.py` (largest _history_ops 207 LOC) + `git_engine.py` shrunk 849 → **256 LOC** + ADR-046-addendum1.md (new) + test_git_engine_package_layout.py (new) + scripts/check_god_files.py | D1/D2 files, other core/versioning/ files | #1472 / [PR #1477](https://github.com/zjzcpj/SciStudio/pull/1477) base=umbrella/phase3-bucket-d ✓ retargeted; admin-approved:core-change ✓; 66 pass / 0 fail; §C9 grep clean; HeadState repaired to live in git_engine.py (lazy-import in _head_state breaks at-import cycle) | `[x]` done |
...
- [x] D3': `git_engine.py` class-binding split + ADR-046 Addendum 1 written → [PR #1477](https://github.com/zjzcpj/SciStudio/pull/1477)
...
| `2026-05-22` | `D3'` | Earlier abandoned D3 attempt had left an uncommitted decomposition in the worktree (6 sibling modules, ADR addendum, layout test, git_engine.py already split). D3' validated this state, found 2 real bugs: (a) `HeadState` had been moved to `_engine_types.py` and re-exported — griffe excludes aliases from facts, breaking ADR-039's `governs.contracts: scistudio.core.versioning.git_engine.HeadState` (audit.closure + audit.doc_drift failed); (b) ADR-046-addendum1 frontmatter had duplicated `ADR-046 Addendum 1:` prefix in title, failing frontmatter.adr-h1 lint. | D3' fixed both: moved `HeadState` + `MergeResult` back to `git_engine.py`; added lazy-import in `_status_ops._head_state` body to break the at-import cycle; stripped duplicated H1 prefix; also auto-fixed 19 UP037 quoted-annotation issues. | None — corrected in D3' PR before push |
