...
- You are working on the SciStudio repository on a tracking branch for the active cascade — your dispatch will name the cascade and tracking branch.
...
- `src/scistudio/core/types/` (entire core data-type system; ADR-027 D2 freezes the seven core types)
- `src/scistudio/core/storage/` (storage backends — Zarr / Arrow / filesystem / composite frozen)
- `src/scistudio/core/proxy.py` (ViewProxy contract)
- `src/scistudio/blocks/base/` (Block ABC, BlockSpec, PortSpec, state machine — except `blocks/registry.py` where ADR-038 §3.3 explicitly authorizes the `block_version` force-injection)
- `src/scistudio/engine/runners/process_handle.py` + `process_monitor.py` + `platform.py` (ADR-019 process lifecycle)
- `src/scistudio/engine/events.py` (EventBus contract — you may **subscribe**, may NOT add new event types EXCEPT `git.head_changed` per ADR-039 §3.8 + `blocks.reloaded` which already exists)
- `src/scistudio/api/routes/ai_pty.py` — the EXISTING `WS /api/ai/pty/{tab_id}` route is frozen (ADR-034 freeze)
- `src/scistudio/blocks/ai/` — frozen except for the AI Block `run_id` → `block_execution_id` rename in D38-2.4a
- `src/scistudio/blocks/io/` + `blocks/process/` + `blocks/code/` + `blocks/app/` + `blocks/subworkflow/` — block subclasses untouched by ADR-038/039
...
2. **No `pip install -e .` from this worktree** — it pollutes the user's global `scistudio` import. Document any dep change in the PR body so the user runs the install in main.
...
12. **Run the same checks CI runs** locally before push:
    ...
    - `mypy src/scistudio/ --ignore-missing-imports` (where applicable)
