...
- [~] Protected branch and umbrella PR number recorded in this checklist -> PR pending
- [x] No `pip install -e .` environment pollution found -> `python -c "import scistudio"` failed without `PYTHONPATH`
- [~] Dispatch checklist copied from the template and committed -> this file pending commit
- [~] Dispatch prompts created from the correct prompt template and linked below -> prompt files pending commit
