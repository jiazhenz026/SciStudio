...
5. `src/scistudio/api/routes/ai_pty.py:62-226` — the existing user-launched WS PTY route (READ ONLY; you add a sibling endpoint).
6. `src/scistudio/ai/agent/terminal.py:443-535` — `spawn_claude` / `spawn_codex` argv builders (call as-is).
7. `src/scistudio/ai/agent/mcp/_registry.py:33-243` — `ToolEntry` shape + 25-tool catalog (mirror style for `finish_ai_block`).
...
- `src/scistudio/blocks/ai/` — I35a's territory (call AIBlock as opaque consumer; tests use mocks).
- All frontend — I35c's territory.
- The existing `WS /api/ai/pty/{tab_id}` handler — frozen.
- `src/scistudio/core/`, `blocks/base/`, `blocks/registry.py`, `engine/runners/`, `engine/events.py` — frozen. **You MAY emit existing events** (`BLOCK_PAUSED`, `PROCESS_SPAWNED`, `PROCESS_EXITED`) but MAY NOT add new event types.
...
pytest -q --timeout=60
mypy src/scistudio/engine/ src/scistudio/ai/agent/mcp/ --ignore-missing-imports
```
