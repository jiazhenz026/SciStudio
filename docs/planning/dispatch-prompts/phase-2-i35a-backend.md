...
```
Verify scieasy resolves to your worktree. **Do NOT `pip install -e .`** from this worktree.

## STEP 2 — scope IN

You implement (filling stubs left by S35):

- `src/scieasy/blocks/ai/ai_block.py` — full `AIBlock(Block).run()` impl: writes manifest, requests PTY tab via `engine.request_pty_tab()`, transitions to PAUSED, awaits completion signal from `completion.py` watcher, validates outputs via IOBlock loaders, transitions to DONE/ERROR/CANCELLED.
- `src/scieasy/blocks/ai/run_dir.py` — `RunDir` class: per-run `.scieasy/ai-block-runs/{run_id}/` directory creation, manifest writer (atomic temp+rename), transcript copy hook, signal-file path API. Reference `src/scieasy/blocks/app/bridge.py:31-142` for analogous file-exchange pattern.
- `src/scieasy/blocks/ai/completion.py` — `CompletionWatcher` combining: (a) MCP signal file polling under `run_dir/signal/`, (b) `FileWatcher` from `src/scieasy/blocks/app/watcher.py:60-127` on declared `expected_path` outputs, (c) user-button event from engine. Returns first signal that fires, with precedence (a) > (c) > (b) per ADR-035 §3.5.
...
- `src/scieasy/engine/` — that's I35b's territory. Use `engine.request_pty_tab()` etc. as opaque calls; if they don't exist as proper functions yet (only stubs), import and call the stub — your tests use `monkeypatch` to fake the IPC.
- `src/scieasy/ai/agent/mcp/tools_workflow.py` — I35b's territory. Same: import the stub, fake it in tests.
- `src/scieasy/api/routes/` — I35b's territory.
...
mypy src/scieasy/blocks/ai/ --ignore-missing-imports
```
