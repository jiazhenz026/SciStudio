...
- [~] A1-fix1 delivered, pushed at `e54ed1868` (fix `ee0d49ec6`, merge of the moved `feat/2271` `49bef0f75`, ledger `e54ed1868`). All P1/P2 fixed (link-safe delete/move; Windows Job Object + POSIX process-group cancellation reaching orphans; exit detected from the process, bounded drain; reads no longer advance the file version; `search_files` bounded by entries visited + time, stops with its request; owner `isError` decision). P3s fixed: AU3 P3-1..4, 6..10 (P3-5 partial), AU4 P3-3/4/6. Not fixed: AU4 P3-1 (pre-existing FastMCP exception logging), AU4 P3-2 (handled by A3 on #2275), AU4 P3-7 (shared job table / no concurrency limit). Tests on the merged head: 265 passed / 4 skipped (POSIX-only, no-pip, file-symlink privilege). Gate evidence now on the committed diff: 30 changed files (implementation 15, test 8, docs 4, protected_core 2), 9/9 tier-1 checks pass
- [!] Blocker: `guard.core_change_guard` on `src/scistudio/engine/runners/platform.py` (+96: `terminate_job_object`, `job_active_process_count`, `close_job_object` on `PlatformOps`; POSIX no-ops, Windows ctypes) and `process_handle.py` (+12: `ProcessHandle.owns_live_process()` defaulting to the #1542 PID-identity check; `terminate_all` calls it). Manager review: additive; block-worker behavior unchanged (default path identical). Needs the owner's `admin-approved:core-change` label on the Spec 2 PR, or relocation — owner decision pending
- [x] A3 delivered on PR #2275: socket `tools/call` rejects external-audience tools with the unknown-tool METHOD_NOT_FOUND (-32601) shape (`df274ddce`, mypy rename `7173b8eb0`); 3 new tests in `tests/ai/test_mcp_fastmcp.py`; FR-004/US6 spec text updated; ledger base moved to `origin/main` and main merged (`d619247c2`, no conflicts); CI 16/16 at `d7c3fab31` with fastmcp==4.0.3 on 3.11 and 3.13; PR comment posted
...
- [x] Core change: A1 recorded the owner's approval (`--admin-label admin-approved:core-change`, `f203276a3`); `guard.core_change_guard` downgraded to a warning pending the owner applying the label on the PR. A1 merged the moved `feat/2271` (`96437bd52`); pre-PR check on 30 files, 9/9 tier-1 pass (`e63082dee`)
- [~] Owner directive (2026-09-11): open the Spec 2 PR now rather than after #2275 merges — PR against `main`, stacked by content on #2275 (the #2274/#2275 pattern), ledger base-ref stays `feat/2271-webmcp-bridge`; after #2275 merges, merge main into the branch and move the base-ref to main. Supersedes "rebase onto main before opening" (option B)
- [x] PR #2292 opened against main (head `f28dd2ffd`, pre-PR finalize `736e5d590`, post-PR finalize `f28dd2ffd`); wrapper run with `SCISTUDIO_GATE_BASE=origin/feat/2271-webmcp-bridge` (without it the wrapper measured against origin/main and flagged Spec 1 files); no preflight skip
- [~] PR #2292 first CI: 11 pass / 5 fail — Verify Workflow Compliance (core-change label pending, expected); Type Check (mcp 2.2.0 via fastmcp 4.0.3 rejects `CallToolResult(structuredContent=, isError=)`); Test 3.11 (POSIX cancel reaps the asyncio-owned shell → job stays `running`; plus 4 packaging collection errors "distutils already imported", branch-specific, cause unknown); Deferral ratchet ("later" in a comment); CodeQL py/path-injection #274-276 in `_file_writes.py`. Codex review: P1 cancel reaping (same as above), P1 expected-version check not atomic with replace, P1 Windows Job Object failure must refuse the command, P2 search deadline not checked inside a file scan. Fix round A1-fix2 dispatched (prompt saved); owner to apply `admin-approved:core-change` on #2292
- [x] PR #2275 (Spec 1) merged by the owner at `84643e354`; verified landed (0 commits of `feat/2271` outside main; `webmcp.py` on main; #2271 CLOSED). Umbrella #2273 closed unmerged with an evidence comment and #2272 closed manually (option A)
- [!] Session interruption (2026-09-11): the previous session ended while A1 was mid A1-fix2; A1 left 8 files uncommitted and unpushed (+676/-207), no live processes; #2292 CI unchanged at `f28dd2ffd`. A1b dispatched (prompt "A1b") to review the draft hunks, finish A1-fix2, move the ledger base to main and merge origin/main
- [x] A1b delivered A1-fix2 and the move onto main. Head `44ae01036` (fix `e23c75150`, test fix `7f6be2cff`, merge of origin/main `f616075ed`, ledger commits); ledger base-ref moved to main. CI 15/16 green, including CodeQL (alerts #274-276 fixed by code; #277 from a test regex fixed) and both Python versions. The only red is Verify Workflow Compliance = `guard.core_change_guard`, waiting on the owner's `admin-approved:core-change` label. MERGEABLE. The 3.11 collection-error root cause: `importlib.util.find_spec("pip")` inside a `skipif` at collection time triggers setuptools' `_distutils_hack` on 3.11 only. Every new test fails on the old head `f28dd2ffd` and passes now. All four Codex comments were answered with the fix commits
- [~] A1b decisions for owner confirmation: (1) regex search runs in a stdlib-only subprocess (`python -I -S -c`), killed at the time budget or request end (Job Object on Windows, SIGALRM fallback on POSIX), returning `regex_unavailable` if it cannot start; one subprocess per regex search, beyond the dispatched line-level checks, because Python `re` cannot be interrupted. (2) The editor route now returns 403 when the requested path resolves to the project root itself (previously it fell through to the suffix check), and `user_library.py` shares the function; real-file behavior is unchanged
- [x] Manager verification: head, mergeability, check states, `confine_to_project` and the regex worker confirmed on `origin/feat/2279-agent-context-workspace`
...
- [x] Owner live re-check of the fix round (Windows dev launch at `5465bcc79`, 2026-09-11): in pure external-AI mode "Stop and Quit" quits without a confirmation (by design: confirmation only while a desktop window is attached). Owner decision: **keep as is** (confirmation only with a desktop window attached)
- [x] Owner decisions (2026-09-11) on the A2-fix1 leftovers: AU1 P3-8 (a switched desktop session relaunches in external-AI mode without a main window) **accepted**; macOS/Linux installed-build verification **after merge** (spec stays Draft until the owner verifies; issues found become bugfixes); AU1 P3-12 user-guide page **tracked as a follow-up issue**
